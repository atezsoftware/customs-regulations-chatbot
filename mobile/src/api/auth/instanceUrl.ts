...
export function normalizeServerUrl(input: string): string {
  ...
  if (trimmed.length === 0) {
    throw new Error("Enter your Atez Customs Assistant instance URL.");
  }
...
// Reject non-Atez Customs Assistant responses (captive portal, HTML error page) so we don't commit a dead URL.
...
export async function probeAuthType(
  ...
  try {
    ...
    throw new Error("Couldn't reach an Atez Customs Assistant instance at that address.");
  } finally {
  ...
  if (!isAuthTypeMetadata(body)) {
    throw new Error("That address doesn't look like an Atez Customs Assistant instance.");
  }
  return body;
}
