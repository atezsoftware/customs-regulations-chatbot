...
test.describe("LLM Runtime Selection", () => {
  ...
  test("model selection persists across refresh and subsequent messages in the same chat", async ({
    ...
    await verifyCurrentModel(page, selectedModelDisplay);
    const firstAnswerProvenance = page
      .getByTestId("AgentMessage/model-provenance")
      .first();
    await expect(firstAnswerProvenance).toContainText("Generated by");
    await expect(firstAnswerProvenance).toContainText(persistenceModelName);
    ...
    await verifyCurrentModel(page, selectedModelDisplay);
    const reloadedAnswerProvenance = page
      .getByTestId("AgentMessage/model-provenance")
      .first();
    await expect(reloadedAnswerProvenance).toContainText("Generated by");
    await expect(reloadedAnswerProvenance).toContainText(persistenceModelName);
