...
function buildItems(
  ...
  if (!isCurator) {
    add(SECTIONS.DOCUMENTS_AND_KNOWLEDGE, ADMIN_ROUTES.AMENDMENTS);
    add(SECTIONS.DOCUMENTS_AND_KNOWLEDGE, ADMIN_ROUTES.BENCHMARK);
  }
  if (!isCurator) {
    items.push({
...
export default function AdminSidebar() {
  ...
  const hasUnlockedSelfHostedFeatures =
    !NEXT_PUBLIC_CLOUD_ENABLED && settings?.ee_features_enabled === true;
  // Default to true while loading to avoid flashing "Upgrade Plan"
  const hasSubscriptionOrLicense =
    hasUnlockedSelfHostedFeatures || billingLoading || licenseLoading
      ? true
      : Boolean(
                ...
                <SidebarTab
                  ...
                  tooltip={markdown(
                    requiredTier === Tier.ENTERPRISE
                      ? "This feature is available on the [Enterprise version of Atez Customs Assistant](/admin/billing) only."
                      : "This feature is available on the [Business or Enterprise version of Atez Customs Assistant](/admin/billing) only."
                  )}
                >
                  {name}
                </SidebarTab>
