...
export async function getAmendmentRuntime(
  batchId: number
): Promise<AmendmentRuntime> {
  const response = await fetch(
    `/api/regulatory/amendments/batches/${batchId}/runtime`
  );
  if (!response.ok) {
    handleRequestError("Get amendment runtime", response);
  }
  return response.json();
}
