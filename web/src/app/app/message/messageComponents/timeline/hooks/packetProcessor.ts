...
import { CitationChunkMap, CitationMap } from "@/app/app/interfaces";
...
export interface ProcessorState {
  ...
  citationChunkMap: CitationChunkMap;
...
function handleCitationPacket(state: ProcessorState, packet: Packet): void {
  ...
  state.citationChunkMap[citationInfo.citation_number] = citationInfo.chunk_ind;

  // Also add to citations array for CitedSourcesToggle (deduplicated)
  const citationIdentity = `${citationInfo.document_id}:${citationInfo.chunk_ind ?? "document"}`;
  if (!state.seenCitationDocIds.has(citationIdentity)) {
    state.seenCitationDocIds.add(citationIdentity);
  ...
  if (packet.obj.type === PacketType.SEARCH_TOOL_DOCUMENTS_DELTA) {
    ...
    if (docDelta.documents) {
      for (const doc of docDelta.documents) {
        if (doc.document_id) {
          state.documentMap.set(
            `${doc.document_id}:${doc.chunk_ind ?? 0}`,
            doc
          );
        }
      }
