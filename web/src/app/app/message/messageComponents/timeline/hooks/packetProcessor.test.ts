...
describe("packetProcessor", () => {
  describe("createInitialState", () => {
    ...
    test("initializes empty citationMap", () => {
      ...
      expect(state.citationChunkMap).toEqual({});
    });
  ...
  describe("Citation flow", () => {
    test("keeps different chunks from the same document distinct", () => {
      const state = createInitialState(1);
      const packets = [
        createCitationPacket(1, "shared-doc", {}, 7, "Act > Article 7"),
        createCitationPacket(2, "shared-doc", {}, 12, "Act > Article 12"),
      ];
      ...
      expect(result.citationChunkMap).toEqual({ 1: 7, 2: 12 });
  ...
  describe("Search Tool flow", () => {
    ...
    test("SEARCH_TOOL_DOCUMENTS_DELTA extracts documents to documentMap", () => {
      ...
      expect(result.documentMap.get("doc-1:0")).toBeDefined();
      expect(result.documentMap.get("doc-2:0")).toBeDefined();
    });
    ...
    test("full search flow: START -> QUERIES -> DOCUMENTS -> SECTION_END", () => {
      ...
      expect(result.groupKeysWithSectionEnd.has("0-0")).toBe(true);
      expect(result.documentMap.has("doc-1:0")).toBe(true);
    });
    ...
    test("multiple DOCUMENTS_DELTA packets accumulate documents", () => {
      ...
      expect(result.documentMap.has("doc-1:0")).toBe(true);
      expect(result.documentMap.has("doc-2:0")).toBe(true);
    });
    ...
    test("parallel search tools at same turn_index with different tab_index", () => {
      ...
      expect(result.documentMap.has("doc-a:0")).toBe(true);
      expect(result.documentMap.has("doc-b:0")).toBe(true);
    });
  });
  ...
  describe("Fetch Tool flow", () => {
    ...
    test("FETCH_TOOL_DOCUMENTS extracts documents to documentMap", () => {
      ...
      expect(result.documentMap.has("fetched-doc-1:0")).toBe(true);
    });
  ...
  describe("multi-tool scenarios", () => {
    test("Search + Python + Fetch in same conversation", () => {
      ...
      expect(result.potentialDisplayGroups.length).toBe(1);
      expect(result.documentMap.has("search-doc:0")).toBe(true);
      expect(result.documentMap.has("fetch-doc:0")).toBe(true);
      expect(result.finalAnswerComing).toBe(true);
      expect(result.stopPacketSeen).toBe(true);
    });
