...
export default function ConfigureProviderModal({
  ...
  const associationPatch = existingAssociationDirty
    ...
    : {};
  // Managed built-ins (cloud): Atez Customs Assistant owns creds/config, so the modal only edits
  ...
  return (
    <ActionPolicyEditorModal
      ...
      description={
        managed
          ? "Provided by Atez Customs Assistant — configure what the agent may do."
          : descriptor.setup_instructions
      }
      note={
        managed
          ? "This app is provided by Atez Customs Assistant — credentials are managed for you. Choose what the agent may do below. Users connect it from the Apps page."
          : undefined
      }
      fields={fields}
