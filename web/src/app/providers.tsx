...
export function initPostHog(key: string, host?: string | null): void {
  const upstreamConnectionsDisabled =
    process.env.NEXT_PUBLIC_DISABLE_ONYX_UPSTREAM_CONNECTIONS?.toLowerCase() !==
    "false";
  if (upstreamConnectionsDisabled) return;
...
export function PHProvider({ children }: PHProviderProps) {
  useEffect(() => {
    // Build-time key (Atez Customs Assistant Cloud); otherwise PostHogRuntimeInitializer handles it.
