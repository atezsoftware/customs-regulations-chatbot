...
export function ProjectsProvider({ children }: ProjectsProviderProps) {
  ...
  const documentImportEnabled = settingsData.document_import_enabled !== false;
  ...
  const { data: recentFilesData, mutate: mutateRecentFiles } = useSWR<
    ProjectFile[]
  >(isAdmin ? "/api/user/files/recent" : null, errorHandlingFetcher, {
  ...
  const beginUpload = useCallback(
    async (
      ...
      if (!documentImportEnabled) {
        toast.error(
          "Document import is handled by the separate importer deployment."
        );
        onFailure?.([]);
        return [];
      }
