...
test.each(["URL", ...sourceUploadCases.map((source) => source.mode)] as const)(
...
test("analyzes pasted text directly and still prepares a switched-to URL", async () => {
  ...
  mockedCreateAmendmentSourcePackage.mockResolvedValue({
    id: "url-preparation",
    document_set_id: 7,
    status: "ready",
    asset_count: 1,
  ...
  expect(mockedCreateAmendmentSourcePackage).toHaveBeenCalledTimes(1);
});
...
test("queues pasted text and polls durable progress", async () => {
  const progressResponse = await mockedGetAmendmentAnalysis(42);
  mockedGetAmendmentAnalysis.mockResolvedValue({
    ...progressResponse,
    batch: { ...progressResponse.batch, matched_instruction_count: 3 },
  });
  const user = setupUser();
  render(<AmendmentsPage />);
...
test.each(["queued", "paused"] as const)(
  "shows resource %s without a content warning",
  async (status) => {
    const batch = {
      ...queuedBatch,
      status,
      stage: "waiting_resources" as const,
      instruction_count: 5,
      processed_instruction_count: 2,
      matched_instruction_count: 4,
    };
    mockedListAmendmentBatches.mockResolvedValue([batch]);
    mockedGetAmendmentAnalysis.mockResolvedValue({
      batch,
      proposals: [],
      unmatched_instructions: [],
