...
class ChunkReranker:
    ...
    def rerank(
        ...
        or `None` on any failure. Short-circuits (no network call) for 0-1
        documents (a relevance_score of 1.0 there is a placeholder — nothing
        downstream compares it across a single-document result).
        ...
        try:
            response = self._client.post(
                ...
                json={
                    ...
                    "query": query,
                    "documents": documents,
                    "top_n": max(min(top_n, len(documents)), 1),
                },
            )
