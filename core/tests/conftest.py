...
without making actual API calls, and a `database_url` fixture for tests that
need a real Postgres (indexing/search/storage) — these require a running
Postgres reachable via the DATABASE_URL env var (the same instance `db/`
brings up; point it at a throwaway test database) and skip otherwise.
"""
...
def make_mock_llm_client():
    """Build a `GeminiLLMClient` wrapping `MockGenAIClient`, for agent tests."""
    from fs_explorer.llm import GeminiLLMClient
