...
import fs_explorer_indexer.indexing.metadata as metadata_module
...
from fs_explorer_indexer.indexing.chunker import SmartChunker
from fs_explorer_indexer.indexing.metadata import (
    auto_discover_profile,
    normalize_langextract_profile,
)
from fs_explorer_indexer.indexing.pipeline import IndexingPipeline, SourceDocument
from fs_explorer_indexer.indexing.schema import SchemaDiscovery
...
def test_schema_discovery_with_langextract_fields(tmp_path: Path, monkeypatch) -> None:
    ...
    from fs_explorer_indexer.indexing.metadata import default_langextract_profile

    monkeypatch.setattr(
        "fs_explorer_indexer.indexing.schema.auto_discover_profile",
...
def test_index_documents_uses_real_file_path_for_manifest_metadata(
    tmp_path: Path,
    monkeypatch,
    database_url: str,
) -> None:
    raw_file = tmp_path / "stored-upload.md"
    raw_file.write_text("TIR carnet guarantee protocol text.")
    display_name = (
        "07.06.2010_12647_roder_kefil_kurulus_ile_gumruk_mustesarligi_"
        "arasinda_imzalanan_protokol_x1rodernet_turunde_teminatx2.docx"
    )
    ...
    storage = PostgresStorage(database_url)
    result = IndexingPipeline(storage=storage).index_documents(
        corpus_root="/__customs_regulations__/directories/999",
        documents=[
            SourceDocument(
                file_path=str(raw_file),
                relative_path=f"7-{display_name}",
    ...
    assert metadata["filename"] == display_name
    assert metadata["relative_path"] == f"7-{display_name}"
    assert stored["absolute_path"] == display_name
...
def test_auto_discover_profile_with_mock_llm(
    ...
    with patch(
        "fs_explorer_indexer.indexing.metadata._get_genai_client",
