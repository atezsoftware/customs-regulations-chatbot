...
from .index_config import resolve_database_url
...
def _internal_token_valid(provided: str | None) -> bool:
    """
    Check a caller-supplied token against `CORE_INTERNAL_TOKEN`.

    core is meant to be called only by the backend, never directly from a
...
def _corpus_root(value: str) -> str:
    """Normalize physical or virtual corpus identifiers the same way storage does."""
...
async def index_status(folder: str, database_url: str | None = None):
    ...
    try:
        corpus_root = _corpus_root(folder)
    ...
    try:
        ...
        if use_index:
            ...
            for index_folder in index_folders:
                corpus_root = _corpus_root(index_folder)
