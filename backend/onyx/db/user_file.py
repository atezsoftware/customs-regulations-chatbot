...
def get_user_file_ids_for_user_batch(
    ...
    up_to_id: str | None,
    *,
    exclude_regulatory_files: bool = False,
) -> list[str]:
    ...
    if exclude_regulatory_files:
        stmt = stmt.where(~exists().where(RegulatoryChunk.user_file_id == UserFile.id))
...
def mark_regulatory_user_files_reconcile_pending__no_commit(
    db_session: Session,
) -> int:
    """Queue canonical PG→FUTURE projection for every completed regulatory file."""

    regulatory_file_ids = select(RegulatoryChunk.user_file_id).distinct()
    result = db_session.execute(
        update(UserFile)
        .where(
            UserFile.id.in_(regulatory_file_ids),
            UserFile.status == UserFileStatus.COMPLETED,
        )
        .values(secondary_reconcile_pending=True)
    )
