...
def check_project_write_access(
    project_id: int, user: User, db_session: Session
) -> bool:
    return check_project_access(project_id, user, db_session)


def check_project_access(project_id: int, user: User, db_session: Session) -> bool:
    if user.role == UserRole.ADMIN:
        return db_session.get(UserProject, project_id) is not None

    return check_project_ownership(project_id, user.id, db_session)
