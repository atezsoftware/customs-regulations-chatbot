...
class RegulatoryChunk(Base):
    """A structure-aware chunk of a regulatory user file.
    ...
    lower boundary; NULL end = still valid. Source snapshot dates must be set
    explicitly rather than inferred from descriptive document metadata. An
    amendment closes the old chunk's window
    (validity_end_date = new chunk's validity_start_date) and links the two
    via the supersession columns — nothing is deleted, so as-of-date search
    can serve historical questions.
    """

    __tablename__ = "regulatory_chunk"
    ...
    heading_path: Mapped[list[str]] = mapped_column(
        PGJSONB, nullable=False, default=list
    )
    chunk_metadata: Mapped[dict[str, Any]] = mapped_column(
        PGJSONB, nullable=False, default=dict
    )
    validity_start_date: Mapped[datetime.date | None] = mapped_column(
        Date, nullable=True
    )
    validity_end_date: Mapped[datetime.date | None] = mapped_column(Date, nullable=True)
    status: Mapped[str] = mapped_column(
        Text, nullable=False, default=RegulatoryChunkStatus.ACTIVE.value
    )
    ...
    superseded_by_chunk_id: Mapped[str | None] = mapped_column(
        ForeignKey("regulatory_chunk.id", ondelete="SET NULL"), nullable=True
    )
    created_at: Mapped[datetime.datetime] = mapped_column(
        DateTime(timezone=True), nullable=False, server_default=func.now()
...
class AmendmentBatch(Base):
    ...
    Nothing writes to `regulatory_chunk` until a proposal is approved.
    """
...
class AmendmentProposal(Base):
    """One atomic, individually approvable/rejectable amendment.
    ...
    id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
    batch_id: Mapped[int] = mapped_column(
        ForeignKey("amendment_batch.id", ondelete="CASCADE"), nullable=False
    )
    ...
    old_chunk_snapshot: Mapped[dict[str, Any]] = mapped_column(
        PGJSONB, nullable=False, default=dict
    )
    new_chunk_draft: Mapped[dict[str, Any]] = mapped_column(
        PGJSONB, nullable=False, default=dict
    )
    chunk_changes: Mapped[list[dict[str, Any]]] = mapped_column(
        PGJSONB, nullable=False, default=list, server_default=text("'[]'::jsonb")
    )
    match_confidence: Mapped[float | None] = mapped_column(Float, nullable=True)
    match_rationale: Mapped[str | None] = mapped_column(Text, nullable=True)
    date_rationale: Mapped[str | None] = mapped_column(Text, nullable=True)
    status: Mapped[str] = mapped_column(
        Text, nullable=False, default=AmendmentProposalStatus.PENDING.value
    )
    applied_new_chunk_id: Mapped[str | None] = mapped_column(
        ForeignKey("regulatory_chunk.id", ondelete="SET NULL"), nullable=True
    )
    applied_new_chunk_ids: Mapped[list[str]] = mapped_column(
        PGJSONB, nullable=False, default=list, server_default=text("'[]'::jsonb")
    )
    ...
    batch: Mapped["AmendmentBatch"] = relationship(
        "AmendmentBatch", back_populates="proposals"
    )
...
class BenchmarkQuestion(Base):
    __tablename__ = "benchmark_question"
...
class BenchmarkRun(Base):
    ...
    report_output_tokens: Mapped[int | None] = mapped_column(Integer, nullable=True)
    report_cost_cents: Mapped[float | None] = mapped_column(Float, nullable=True)
    created_by: Mapped[UUID | None] = mapped_column(
        ForeignKey("user.id", ondelete="SET NULL"), nullable=True
    )
    ...
    items: Mapped[list["BenchmarkRunItem"]] = relationship(
        "BenchmarkRunItem", back_populates="run", cascade="all, delete-orphan"
    )
...
class BenchmarkRunItem(Base):
    ...
    cost_source: Mapped[str] = mapped_column(
        Text, nullable=False, default=BenchmarkCostSource.UNAVAILABLE.value
    )
    ...
    run: Mapped["BenchmarkRun"] = relationship("BenchmarkRun", back_populates="items")
    question: Mapped["BenchmarkQuestion"] = relationship(
        "BenchmarkQuestion", back_populates="run_items"
    )
    judgment: Mapped["BenchmarkRunJudgment | None"] = relationship(
    ...
    __table_args__ = (
        Index("ix_benchmark_run_item_run_id", "run_id"),
        Index("ix_benchmark_run_item_question_id", "question_id"),
        CheckConstraint(
...
class BenchmarkRunJudgment(Base):
    __tablename__ = "benchmark_run_judgment"
    ...
    run_item: Mapped["BenchmarkRunItem"] = relationship(
        "BenchmarkRunItem", back_populates="judgment"
    )
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "cost_source IN ('measured', 'unavailable')",
            name="benchmark_judgment_cost_source_check",
        ),
    )
