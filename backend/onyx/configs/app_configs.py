...
from shared_configs.configs import DISABLE_ONYX_UPSTREAM_CONNECTIONS, MULTI_TENANT
...
# Deployment boundary for source-document ingestion. Lightweight production
# deployments keep indexed documents searchable while moving parsing and
# indexing to a separately operated importer runtime.
...
POSTHOG_API_KEY = (
    None if DISABLE_ONYX_UPSTREAM_CONNECTIONS else os.environ.get("POSTHOG_API_KEY")
)
...
# Domain used to build outgoing Message-ID headers. Defaults to the sender's own
# domain so messages are attributed to this deployment rather than a vendor.
EMAIL_MESSAGE_ID_DOMAIN = (
    os.environ.get("EMAIL_MESSAGE_ID_DOMAIN")
    or (EMAIL_FROM.split("@", 1)[1] if "@" in EMAIL_FROM else "")
    or "localhost"
)
...
# for larger installations, while the cross-product cap remains the final guard
# against an accidental burst of provider calls.
REGULATORY_BENCHMARK_MAX_CANDIDATES = max(
...
REGULATORY_INDEXING_OPENROUTER_BATCH_URL = os.environ.get(
    "REGULATORY_INDEXING_OPENROUTER_BATCH_URL",
    "https://openrouter.ai/api/beta/batches",
)
REGULATORY_INDEXING_OPENROUTER_BATCH_MAX_REQUESTS = max(
    1,
    int(os.environ.get("REGULATORY_INDEXING_OPENROUTER_BATCH_MAX_REQUESTS") or 1_000),
)
REGULATORY_INDEXING_OPENROUTER_BATCH_MAX_INPUTS = int(
    os.environ.get("REGULATORY_INDEXING_OPENROUTER_BATCH_MAX_INPUTS") or 45_000
)
REGULATORY_INDEXING_OPENROUTER_BATCH_MAX_BYTES = max(
    1,
    int(
        os.environ.get("REGULATORY_INDEXING_OPENROUTER_BATCH_MAX_BYTES")
        or 32 * 1024 * 1024
    ),
)
REGULATORY_INDEXING_OPENROUTER_BATCH_HORIZON_SECONDS = max(
    1,
    int(
        os.environ.get("REGULATORY_INDEXING_OPENROUTER_BATCH_HORIZON_SECONDS") or 86_400
    ),
)
REGULATORY_INDEXING_ALLOW_ONLINE_EMBEDDING_FALLBACK = (
    os.environ.get(
        "REGULATORY_INDEXING_ALLOW_ONLINE_EMBEDDING_FALLBACK", "false"
    ).lower()
    == "true"
)
...
# provider. Transient failures are retried locally before the indexing task decides
# whether enrichment is complete.
CONTEXTUAL_RAG_MAX_WORKERS = max(
    1, _non_negative_int_env("CONTEXTUAL_RAG_MAX_WORKERS", 2)
)
CONTEXTUAL_RAG_MAX_RETRIES = _non_negative_int_env("CONTEXTUAL_RAG_MAX_RETRIES", 2)
CONTEXTUAL_RAG_LLM_TIMEOUT_SECONDS = max(
    1, _non_negative_int_env("CONTEXTUAL_RAG_LLM_TIMEOUT_SECONDS", 60)
)
CONTEXTUAL_RAG_RETRY_BASE_SECONDS = max(
    0.0, float(os.environ.get("CONTEXTUAL_RAG_RETRY_BASE_SECONDS") or 1.0)
)
CONTEXTUAL_RAG_RETRY_MAX_SECONDS = max(
    CONTEXTUAL_RAG_RETRY_BASE_SECONDS,
    float(os.environ.get("CONTEXTUAL_RAG_RETRY_MAX_SECONDS") or 8.0),
)
...
# Anonymous usage telemetry. Off unless a deployment explicitly opts in with
# DISABLE_TELEMETRY=false, so no usage data leaves the deployment by default.
DISABLE_TELEMETRY = DISABLE_ONYX_UPSTREAM_CONNECTIONS or (
    os.environ.get("DISABLE_TELEMETRY", "").lower() != "false"
)
