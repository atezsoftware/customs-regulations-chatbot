...
from onyx.tools.built_in_tools import BUILT_IN_TOOL_MAP, get_built_in_tool_by_id
...
def _construct_tools_impl(
    ...
    for db_tool_model in configured_tools:
        ...
        if db_tool_model.in_code_tool_id:
            # A persona created before code execution / internet access were
            # removed from this deployment may still reference one of those
            # tool ids; the migration detaches them, but skip defensively
            # rather than raise if any slip through.
            if db_tool_model.in_code_tool_id not in BUILT_IN_TOOL_MAP:
                logger.debug(
                    "Skipping unknown built-in tool id %s (removed from registry)",
