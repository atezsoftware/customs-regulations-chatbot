...
from onyx.server.features.regulatory.api import router as regulatory_router
from onyx.server.features.regulatory.benchmark_api import (
    router as regulatory_benchmark_router,
)
...
from onyx.server.manage.reranking.api import admin_router as reranking_admin_router
...
def get_application(lifespan_override: Lifespan | None = None) -> FastAPI:
    application = FastAPI(
        title="Atez Customs Assistant Backend",
        version=__version__,
        description="Atez Customs Assistant API for AI-powered chat with search, document indexing, agents, actions, and more",
        servers=[
            {
                "url": f"{WEB_DOMAIN.rstrip('/')}/api",
                "description": "Atez Customs Assistant API Server",
            }
        ],
    ...
    include_router_with_global_prefix_prepended(application, regulatory_router)
    include_router_with_global_prefix_prepended(
        application, regulatory_benchmark_router
    )
    ...
    include_router_with_global_prefix_prepended(application, reranking_admin_router)
