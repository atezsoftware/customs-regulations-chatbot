...
class BaseFilters(BaseModel):
    ...
    # Regulatory temporal filter: only chunks valid on this calendar date.
    # Chunks that never carry validity_start_date/validity_end_date (every
    # non-regulatory chunk, and any regulatory chunk that was never amended)
    # are unaffected — the filter only excludes a chunk that explicitly
    # states it wasn't valid yet or is no longer valid on this date. None
    # disables the filter entirely (matches every chunk regardless of
    # validity window).
    as_of_date: date | None = None

    # Internal deployment scope: the default regulatory assistant searches only
    # structure-aware chunks backed by PostgreSQL regulatory_chunk rows. This
...
class BasicChunkRequest(BaseModel):
    ...
    # For pure full-text retrieval, require high analyzed-term coverage. This
    # is ignored by semantic/hybrid retrieval.
...
class InferenceChunk(BaseChunk):
    ...
    # Regulatory projection identity and validity. These are absent for every
    # generic chunk and remain optional for chunks read from older indices.
...
def _metadata_with_regulatory_fields(
    chunk: InferenceChunk,
) -> dict[str, str | list[str]]:
    """Preserve chunk-level regulatory identity across SearchDoc persistence."""
    metadata = dict(chunk.metadata)
    if chunk.regulatory_chunk_id is not None:
        metadata["regulatory_chunk_id"] = chunk.regulatory_chunk_id
    if chunk.heading_path is not None:
        metadata["regulatory_heading_path"] = list(chunk.heading_path)
    if chunk.validity_start_date is not None:
        metadata["regulatory_validity_start_date"] = (
            chunk.validity_start_date.isoformat()
        )
    if chunk.validity_end_date is not None:
        metadata["regulatory_validity_end_date"] = chunk.validity_end_date.isoformat()
    return metadata
    ...
    def from_chunks_or_sections(
        ...
        search_docs = [
            cls(
                ...
                boost=chunk.boost,
                hidden=chunk.hidden,
                metadata=_metadata_with_regulatory_fields(chunk),
                score=chunk.score,
...
class SearchDocsResponse(BaseModel):
    ...
    # Maps the citation number to the document id. `citation_chunk_mapping`
    # disambiguates multiple cited chunks from the same document while keeping
    # this field wire-compatible with integrations that only know document ids.
    citation_mapping: dict[int, str]
    citation_chunk_mapping: dict[int, int] = Field(default_factory=dict)
