...
def _embed_and_hybrid_search(
    ...
    try:
        ...
        # ordinary enterprise search retains its existing failure semantics.
        if not query_request.filters.regulatory_chunks_only:
            raise
        logger.warning(
...
def _keyword_search(
    ...
    return document_index.keyword_retrieval(
        ...
        filters=query_request.filters,
        num_to_retrieve=query_request.limit or NUM_RETURNED_HITS,
        high_term_coverage=query_request.high_term_coverage,
    )
...
def search_chunks(
    ...
    if normal_search_enabled:
        if query_request.hybrid_alpha is not None and query_request.hybrid_alpha == 0.0:
            ...
            # only set by Elasticsearch-aware producers; Vespa would raise
