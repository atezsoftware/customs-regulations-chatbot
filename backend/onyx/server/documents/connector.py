...
def submit_connector_request(
    ...
    if MULTI_TENANT and not DISABLE_ONYX_UPSTREAM_CONNECTIONS:
        mt_cloud_telemetry(
            tenant_id=tenant_id,
            distinct_id=str(user.id),
    ...
    if EMAIL_CONFIGURED and not DISABLE_ONYX_UPSTREAM_CONNECTIONS:
        try:
...
<li><strong>Tenant ID:</strong> {tenant_id}</li>
    ...
    message = (
        "Connector request recorded locally; vendor delivery is disabled."
        if DISABLE_ONYX_UPSTREAM_CONNECTIONS
        else "Connector request submitted successfully. We'll prioritize popular requests!"
    )
