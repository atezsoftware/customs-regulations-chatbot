...
class LabelingVertexBatchGateway(GoogleVertexBatchGateway):
    ...
    def _get_owned_job_state(self, remote_job_name: str) -> VertexBatchState:
        ...
        if job.name != remote_job_name:
            raise VertexBatchContractError(
                "Vertex labeling lookup returned a different job"
            )
        display_match = (
            _SUBMISSION_KEY.fullmatch(job.display_name)
            if isinstance(job.display_name, str)
            else None
        )
        if display_match is None:
            raise VertexBatchContractError(
                "Vertex labeling job has an invalid submission identity"
            )
        submission_hash = display_match.group(1)
        ...
        if job.src is None or job.src.gcs_uri != [input_uri]:
            raise VertexBatchContractError(
                "Vertex labeling job has an unexpected input URI"
            )
        if job.dest is None or job.dest.gcs_uri != output_uri:
            raise VertexBatchContractError(
                "Vertex labeling job has an unexpected destination URI"
            )
    ...
    def get(self, remote_job_name: str) -> VertexBatchState:
        return self._get_owned_job_state(remote_job_name)
        ...
        with _translate_gateway_errors():
            ...
            with self._managed_storage_client(credentials) as storage_client:
                blobs = storage_client.list_blobs(
                    bucket_name,
                    prefix=f"{prefix.rstrip('/')}/labeling/",
                    max_results=1,
                    timeout=self._request_timeout_seconds,
                )
    ...
    def cancel(self, remote_job_name: str) -> None:
        self._get_owned_job_state(remote_job_name)
