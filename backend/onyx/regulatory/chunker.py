"""Structure-aware chunking for Turkish regulatory documents.

Ported from the customs-regulations-chatbot reference implementation
(`fs_explorer_indexer.indexing.regulatory_chunker`). Operates on markdown /
plaintext already extracted by Onyx's file processing — the file-parsing entry
points of the original module are intentionally not ported. Entry point:
`RegulatoryChunker.chunk_text()`.
"""
...
_HEADER_DOCUMENT_DATE_LABEL_RE = re.compile(
    r"^(?:"
    r"resmi\s+gazete(?:nin|['’]nin)?\s+tarih(?:i)?|"
    r"(?:belge|dokuman|yayim|yayin|yayimlanma|yayinlanma|karar|kabul)\s+"
    r"tarih(?:i)?|"
    r"tarih(?:i)?"
    r")\b(?P<remainder>.*)$"
)
_BASE_PUBLICATION_DATE_LABEL_RE = re.compile(
    r"\b(?:kanunun|yonetmeligin|tebligin|genelgenin|yonergenin|kararin|"
    r"sozlesmenin|andlasmanin|protokolun)\s+yayi[mn]landigi\s+"
    r"resmi\s+gazete(?:nin|['’]nin)?\s+tarihi\b"
)
_DOCUMENT_METADATA_COLUMN_RE = re.compile(
    r"^(?:sayisi|sayi|numarasi|numara|no)\b(?P<remainder>.*)$", re.DOTALL
)
_DOCUMENT_TYPE_BODY_BOUNDARY_RE = re.compile(
    r"^(?:amac|kapsam|dayanak|tanimlar|madde\b|gecici\s+madde\b|"
    r"[a-z0-9\s]+\s+(?:bolum|kisim)\b|yururluk\b|yurutme\b)"
)
_DOCUMENT_TITLE_TRAILING_ANNOTATIONS = (
    r"(?:\s*(?:\([^()\n]{0,120}\)|\[[^\]\n]{0,120}\]))*\s*$"
)
_DOCUMENT_TITLE_TYPE_PATTERNS: tuple[tuple[str, re.Pattern[str]], ...] = (
    (
        "kanun",
        re.compile(rf"\b(?:kanun|kanunu){_DOCUMENT_TITLE_TRAILING_ANNOTATIONS}"),
    ),
    (
        "sozlesme",
        re.compile(
            rf"\b(?:sozlesme|sozlesmesi|andlasma|andlasmasi)"
            rf"{_DOCUMENT_TITLE_TRAILING_ANNOTATIONS}"
        ),
    ),
    (
        "yonetmelik",
        re.compile(
            rf"\b(?:yonetmelik|yonetmeligi){_DOCUMENT_TITLE_TRAILING_ANNOTATIONS}"
        ),
    ),
    (
        "teblig",
        re.compile(rf"\b(?:teblig|tebligi){_DOCUMENT_TITLE_TRAILING_ANNOTATIONS}"),
    ),
    (
        "genelge",
        re.compile(rf"\b(?:genelge|genelgesi){_DOCUMENT_TITLE_TRAILING_ANNOTATIONS}"),
    ),
    (
        "yonerge",
        re.compile(rf"\b(?:yonerge|yonergesi){_DOCUMENT_TITLE_TRAILING_ANNOTATIONS}"),
    ),
    (
        "karar",
        re.compile(rf"\b(?:karar|karari){_DOCUMENT_TITLE_TRAILING_ANNOTATIONS}"),
    ),
    (
        "protokol",
        re.compile(rf"\b(?:protokol|protokolu){_DOCUMENT_TITLE_TRAILING_ANNOTATIONS}"),
    ),
)
...
_ARTICLE_REFERENCE_CONTINUATION_RE = re.compile(
    r"^(?:uyarinca|geregince|kapsaminda|cercevesinde|dogrultusunda|"
    r"bakimindan|hukum(?:ler)?(?:i|in|ine|leri|lerinin|lerine)?|"
    r"fikra(?:si|sinin|sina)?|bent(?:i|inin|ine)?)\b"
)
...
_APPENDIX_IDENTIFIER_PATTERN = r"(?:[a-z]+\d+[a-z]?|\d+[a-z]?|[ivxlcdm]+[a-z]?|[a-z])"
_APPENDIX_RE = re.compile(
    rf"^(?P<label>ek)(?:\s*[-:]\s*|\s+)"
    rf"(?P<num>{_APPENDIX_IDENTIFIER_PATTERN})\b(?P<rest>.*)$|"
    rf"^(?P<label2>ek)\s*$",
    flags=re.IGNORECASE,
)
_SUPPLEMENT_RE = re.compile(
    rf"^(?P<label>ilave)(?:\s*[-:]\s*|\s+)"
    rf"(?P<num>{_APPENDIX_IDENTIFIER_PATTERN})\b(?P<rest>.*)$|"
    rf"^(?P<label2>ilave)\s*$",
    flags=re.IGNORECASE,
)
_ARTICLE_POSSESSIVE_SCOPE_HEADING_RE = re.compile(
    r"^(?:(?:gecici|mukerrer)\s+)?madde"
    r"(?:\s+|\s*[:.]\s*)\d+[a-z]?\s*['’](?:n?[iu]n)\s+\S",
    flags=re.IGNORECASE,
)
_NUMBERED_SECTION_RE = re.compile(r"^(?P<num>\d{1,3})[.)]\s+(?P<title>.+?)(?::)?$")
_DOTTED_SECTION_RE = re.compile(
    r"^(?P<num>\d{1,3}(?:\.\d{1,3}){2,7})\s+(?P<rest>\S.*)$"
)
_DOTTED_REFERENCE_CONTINUATION_RE = re.compile(
    r"^(?:uyarinca|geregince|kapsaminda|cercevesinde|dogrultusunda|bakimindan)\b"
)
...
_DASHED_PAGE_NUMBER_RE = re.compile(r"^[-\u2013\u2014]\s*\d{1,6}\s*[-\u2013\u2014]$")
_BARE_PAGE_NUMBER_RE = re.compile(r"^\d{1,6}$")
_MIN_BARE_PAGE_SEQUENCE_LENGTH = 3
_MIN_BARE_PAGE_DISTANCE_CHARS = 300
_ROMAN_LABELS = {
    "i",
    "ii",
    "iii",
...
class RegulatoryChunker:
    ...
    def _build_drafts(
        ...
        current_appendix_node: StructureNode | None = None
        table_index = 0
        ...
        for block in blocks:
            ...
            if classification["kind"] == "appendix":
                ...
                current_appendix_node = push_node(
                    "appendix",
                    current_appendix,
                    level=1,
                    source_block=block.block_id,
            ...
            if classification["kind"] == "supplement":
                flush()
                supplement_label = str(classification.get("label") or clean_text)
                apply_pending_heading(level=1)
                if current_appendix_node is not None:
                    # An ``İLAVE`` is subordinate to the active ``EK`` even if
                    # intervening KISIM/BÖLÜM nodes have since popped the EK
                    # from the working stack. Restore that known structural
                    # parent instead of inheriting the last article/section.
                    stack = [document_node, current_appendix_node]
                    push_node(
                        "supplement",
                        supplement_label,
                        level=2,
                    ...
                    # Some instruments use İLAVE without an enclosing EK. In
                    # that case it is still a top-level structural boundary.
                    current_appendix = supplement_label
                    push_node(
                        "supplement",
                        supplement_label,
                        level=1,
            ...
            if classification["kind"] == "dotted_section":
                flush()
                dotted_number = str(classification["number"])
                dotted_level = _dotted_section_level(dotted_number)
                if pending_title is not None:
                    apply_pending_heading(level=max(1, dotted_level - 1))
                push_node(
                    "dotted_section",
            ...
            if classification["kind"] == "numbered_section":
                flush()
                title = clean_text
                numbered_section_level = 3
                if current_article_no is not None:
                    # A colon-terminated numbered paragraph can be a parent for
                    # following clauses inside a MADDE. Keep both the staged
                    # topic heading and the numbered parent below that article;
                    # the fixed top-level levels remain appropriate when no
                    # article is active.
                    apply_pending_heading(level=_article_context_level(stack))
                    numbered_section_level = _article_unit_level(
                        stack, kind="paragraph"
                    )
                else:
                ...
                current = _ChunkDraft(
                    ...
                    article_no=current_article_no,
                    article_title=current_article_title,
                    paragraph_no=_paragraph_no(title),
                    appendix_label=current_appendix,
                )
                pending_title = None
                pending_title_block = None
            ...
            if classification["kind"] == "heading":
                ...
                if (
                    ...
                    and _is_probable_article_title(clean_text)
                ):
            ...
            dotted_section_level = _current_dotted_section_level(stack)
            apply_pending_heading(
                level=(
                    _article_context_level(stack)
                    if current_article_no is not None
                    else (
                        dotted_section_level + 1
                        if dotted_section_level is not None
                        else 3
                    )
                )
            )
...
def _markdown_to_blocks(content: str) -> list[SourceBlock]:
    ...
    page_number_line_indexes = _pdf_page_number_line_indexes(text)
    offset = 0
    ...
    for line_index, line in enumerate(lines):
        ...
        offset = end
        stripped = line.strip()
        if line_index in page_number_line_indexes:
            flush_table()
            continue
        if not stripped:
            flush_table()
...
def _pdf_page_number_line_indexes(content: str) -> set[int]:
    """Find page-number artifacts without treating legal footnotes as pages.

    Decorated markers such as ``- 587 -`` are unambiguous. Bare numbers are
    removed only when three or more page-boundary candidates form a consecutive
    sequence and are separated by enough source text to represent real pages.
    """
    ...
    ignored_indexes = {
        ...
        if _DASHED_PAGE_NUMBER_RE.fullmatch(line.strip()) is not None
    }

    bare_candidates: list[tuple[int, int, int]] = []
    offset = 0
    for index, line in enumerate(lines):
        stripped = line.strip()
        is_boundary = (
            ...
            or (index + 1 < len(lines) and not lines[index + 1].strip())
        )
        if is_boundary and _BARE_PAGE_NUMBER_RE.fullmatch(stripped) is not None:
            bare_candidates.append((index, int(stripped), offset))
        offset += len(line)

    run: list[tuple[int, int, int]] = []

    def retain_page_run() -> None:
        if len(run) >= _MIN_BARE_PAGE_SEQUENCE_LENGTH:
            ignored_indexes.update(candidate[0] for candidate in run)
    ...
    for candidate in bare_candidates:
        ...
        previous = run[-1]
        if (
            candidate[1] == previous[1] + 1
            and candidate[2] - previous[2] >= _MIN_BARE_PAGE_DISTANCE_CHARS
        ):
...
def _infer_document_metadata(
    *,
    ...
    document_date = file_date or _infer_labeled_document_date(content)
...
def _infer_document_type(filename: str, header: str) -> str:
    ...
    for line in header.splitlines():
        folded_line = _fold_text(_clean_inline_markdown(line)).strip(" |–—-")
        if not folded_line:
            continue
        if _DOCUMENT_TYPE_BODY_BOUNDARY_RE.match(folded_line):
            break

        meaningful_line_count += 1
        for doc_type, title_pattern in _DOCUMENT_TITLE_TYPE_PATTERNS:
            if title_pattern.search(folded_line):
                return doc_type
        if meaningful_line_count >= 12:
            break
    return "unknown"


def _infer_labeled_document_date(content: str) -> str | None:
    folded_content = _fold_text(content)

    # Official consolidated texts can put their original publication metadata
    # after the legal body.  The possessive document-type phrase distinguishes
    # that table from the following list of amending instruments.
    for match in _BASE_PUBLICATION_DATE_LABEL_RE.finditer(folded_content):
        date = _date_from_metadata_remainder(
            folded_content[match.end() : match.end() + 180]
        )
        if date is not None:
            return date

    # Short labels such as "Tarih" are safe only in the document header.  A
    # whole-body scan would confuse references and amendment histories with
    # the date of the indexed instrument itself.
    header_lines = content.splitlines()[:60]
    for line_index, line in enumerate(header_lines):
        folded_line = _fold_text(_clean_inline_markdown(line)).strip(" |–—-")
        label_match = _HEADER_DOCUMENT_DATE_LABEL_RE.match(folded_line)
        if label_match is None:
            continue

        remainder = label_match.group("remainder")
        date = _date_from_metadata_remainder(remainder)
        if date is not None:
            return date
        if not _metadata_remainder_is_columns_only(remainder):
            continue

        for following_line in header_lines[line_index + 1 : line_index + 3]:
            folded_following_line = _fold_text(
                _clean_inline_markdown(following_line)
            ).strip(" |–—-")
            if not folded_following_line:
                continue
            date = _date_from_metadata_remainder(folded_following_line)
            if date is not None:
                return date
            if not _metadata_remainder_is_columns_only(folded_following_line):
                break
    return None


def _date_from_metadata_remainder(remainder: str) -> str | None:
    candidate = remainder.lstrip(" \t\r\n|:：=_-–—")
    column_match = _DOCUMENT_METADATA_COLUMN_RE.match(candidate)
    if column_match is not None:
        candidate = column_match.group("remainder").lstrip(" \t\r\n|:：=_-–—")
    return _leading_iso_date(candidate)


def _metadata_remainder_is_columns_only(remainder: str) -> bool:
    candidate = remainder.strip(" \t\r\n|:：=_-–—")
    if not candidate:
        return True
    column_match = _DOCUMENT_METADATA_COLUMN_RE.match(candidate)
    return column_match is not None and not column_match.group("remainder").strip(
        " \t\r\n|:：=_-–—"
    )


def _infer_title(content: str) -> str | None:
    page_number_line_indexes = _pdf_page_number_line_indexes(content)
...
def _leading_iso_date(text: str) -> str | None:
    dmy = _DATE_DMY_RE.match(text)
    if dmy:
        day, month, year = (int(dmy.group(1)), int(dmy.group(2)), int(dmy.group(3)))
        if _valid_date_parts(year, month, day):
            return f"{year:04d}-{month:02d}-{day:02d}"

    ymd = _DATE_YMD_RE.match(text)
    if ymd:
        year, month, day = (int(ymd.group(1)), int(ymd.group(2)), int(ymd.group(3)))
        if _valid_date_parts(year, month, day):
            return f"{year:04d}-{month:02d}-{day:02d}"
...
def _dotted_section_level(number: str) -> int:
    return len(number.split("."))


def _dotted_section_label(number: str, rest: str) -> str:
    words = rest.split()
    is_short_title = (
        len(rest) <= 100
        and len(words) <= 12
        and re.search(r"[.;:!?]\s*$", rest) is None
    )
    return f"{number} {rest}" if is_short_title else number
...
def _child_level(stack: list[StructureNode], base_level: int) -> int:
    ...
    # A new article is never a child of the article immediately before it.
    # Ignore that article and all of its descendant context when selecting the
    # structural parent, while retaining a heading/numbered container that was
    # already an ancestor of the previous article.
    current_article_index = next(
        (
            index
            for index in range(len(stack) - 1, -1, -1)
            if stack[index].node_type == "article"
        ),
        None,
    )
    candidate_ancestors = (
        stack[:current_article_index] if current_article_index is not None else stack
    )
    for node in reversed(candidate_ancestors):
...
def _article_context_level(stack: list[StructureNode]) -> int:
    ...
    dotted_section_level = _current_dotted_section_level(stack)
    return dotted_section_level + 1 if dotted_section_level is not None else 2
    ...
    if article_level is not None:
        if kind == "clause":
            paragraph_levels = [
                node.level
                for node in stack
                if node.level > article_level
                and node.node_type in {"paragraph", "numbered_section"}
            ]
            if paragraph_levels:
                return max(paragraph_levels) + 1
        context_levels = [
            node.level
            for node in stack
    ...
    if dotted_section_level is not None:
        if kind == "clause":
            paragraph_levels = [
                node.level
                for node in stack
                if node.level > dotted_section_level
                and node.node_type in {"paragraph", "numbered_section"}
            ]
            if paragraph_levels:
                return max(paragraph_levels) + 1
        return dotted_section_level + 1
...
def _current_dotted_section_level(stack: list[StructureNode]) -> int | None:
    for node in reversed(stack):
        if node.node_type == "dotted_section":
            return node.level
    return None
...
def _classify_block(block: SourceBlock) -> dict[str, Any]:
    ...
    if appendix and _is_probable_appendix_heading(block, clean, appendix):
        label = appendix.group("num") or clean
        return {"kind": "appendix", "label": f"EK {label}".strip()}

    supplement = _SUPPLEMENT_RE.match(folded)
    if supplement and _is_probable_appendix_heading(block, clean, supplement):
        return {"kind": "supplement", "label": clean}

    article = _ARTICLE_RE.match(folded)
    if article and _is_probable_article_heading(block, clean, article):
    ...
    if article and _is_probable_article_scope_heading(block, clean):
        label = str(article.group("label"))
        article_no = article.group("num").upper()
        if "gecici" in label:
            article_no = f"GEÇİCİ {article_no}"
        ...
        return {
            "kind": "article",
            "article_no": article_no,
            "article_heading": clean,
            "rest": "",
        }

    parsed_article_heading = parse_regulatory_article_heading(clean)
    if parsed_article_heading is not None and parsed_article_heading.is_reverse:
        return {
            "kind": "article",
            "article_no": parsed_article_heading.article_no,
            "article_heading": f"MADDE {parsed_article_heading.article_no}",
            "rest": "",
        }
    ...
    dotted_section = _DOTTED_SECTION_RE.match(clean)
    if dotted_section:
        number = dotted_section.group("num")
        rest = dotted_section.group("rest").strip()
        if _is_dotted_reference_continuation(rest):
            return {"kind": "paragraph"}
        return {
            "kind": "dotted_section",
            "number": number,
            "label": _dotted_section_label(number, rest),
        }
    ...
    if (
        block.kind == "heading" or _is_short_bold_line(block.text)
    ) and _is_probable_standalone_heading(clean):
        return {"kind": "heading"}
...
def _is_probable_article_heading(
    block: SourceBlock,
    clean_text: str,
    match: re.Match[str],
) -> bool:
    """Reject prose cross-references that merely begin with ``Madde N``."""
    ...
    if rest.startswith(("'", "’")) or _ARTICLE_REFERENCE_CONTINUATION_RE.match(
        folded_rest
    ):
        return False

    if rest.startswith("("):
        if not _ARTICLE_EXPLICIT_SEPARATOR_RE.match(
            clean_text
        ) and _is_parenthetical_article_reference_continuation(rest):
            return False
        return True
    if _ARTICLE_EXPLICIT_SEPARATOR_RE.match(clean_text):
        return True
    return (
        block.kind == "heading"
        or _is_short_bold_line(block.text)
        or _is_mostly_upper(clean_text)
    )


def _is_probable_article_scope_heading(block: SourceBlock, clean_text: str) -> bool:
    """Recognize a visual heading that scopes content to a named article.

    Turkish instruments commonly introduce an implementation supplement with
    a heading such as ``MADDE N'NİN ...``. It is a structural article anchor,
    while the same words in ordinary prose remain a cross-reference.
    """

    return (
        len(clean_text) <= 180
        and (block.kind == "heading" or _is_short_bold_line(block.text))
        and _is_mostly_upper(clean_text)
        and re.search(r"[.;!?]\s*$", clean_text) is None
        and _ARTICLE_POSSESSIVE_SCOPE_HEADING_RE.match(_fold_text(clean_text))
        is not None
    )


def _is_parenthetical_article_reference_continuation(rest: str) -> bool:
    marker = re.match(r"^\([^)]+\)\s*(?P<continuation>.*)$", rest)
    if marker is None:
        return False
    continuation = marker.group("continuation").lstrip()
    if continuation.startswith(("'", "’")):
        return True
    return (
        _ARTICLE_REFERENCE_CONTINUATION_RE.match(_fold_text(continuation)) is not None
    )
...
def _is_probable_appendix_heading(
    block: SourceBlock, clean_text: str, match: re.Match[str]
) -> bool:
    """Reject prose such as ``Ek 1 kapsamında`` while retaining real EK labels."""
...
def _is_dotted_reference_continuation(rest: str) -> bool:
    """Whether a leading dotted number is the object of following prose."""

    return _DOTTED_REFERENCE_CONTINUATION_RE.match(_fold_text(rest)) is not None


def _is_probable_standalone_heading(text: str) -> bool:
    """Distinguish visual headings from bold or OCR-styled provision text."""
    clean = _clean_inline_markdown(text)
    if not clean or len(clean) > 180:
        return False

    unit = _article_unit(clean, text)
    if unit is not None:
        return _is_structural_article_unit_heading(unit)
    if re.search(r"[.;!?]\s*$", clean):
        return False
    if len(clean) > 90 and not _is_mostly_upper(clean):
        return False
    return _is_probable_article_title(clean)
...
def _drop_non_atomic_parent_drafts(
    ...
    for draft in drafts:
        if (
            ...
            and not _article_draft_has_substantive_content(draft)
        ):
            continue
...
def _article_context_label(draft: _ChunkDraft) -> tuple[str, int | None] | None:
    ...
    if (
        not text
        or (draft.article_title is not None and _same_label(text, draft.article_title))
        or not _is_probable_standalone_heading(text)
    ):
        return None
    return _label_tail(text, limit=140), context_blocks[0].block_id


def _article_draft_has_substantive_content(draft: _ChunkDraft) -> bool:
    """Whether an article draft contains law text, not only its locator/title."""
    if draft.chunk_type != "article" or not draft.blocks:
        return False

    trailing_texts = [
        _clean_inline_markdown(block.text)
        for block in draft.blocks[1:]
        if _clean_inline_markdown(block.text)
    ]
    if trailing_texts:
        if draft.article_title and all(
            _same_label(text, draft.article_title) for text in trailing_texts
        ):
            return False
        return True

    rest = _article_rest_from_original(_clean_inline_markdown(draft.blocks[0].text))
    if not rest:
        return False
    return not (
        draft.article_title is not None and _same_label(rest, draft.article_title)
    )
