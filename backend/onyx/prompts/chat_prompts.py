...
- If sources appear to disagree, first determine from the supplied material whether its source roles, scope, specificity, cross-references, or validity metadata establish which passage directly controls the user's question. When they do, lead with the controlling passage and cite it; mention a non-controlling discrepancy only when it materially affects the answer. If the supplied material does not establish priority, present the unresolved conflict and cite each side. Never silently blend incompatible statements.
