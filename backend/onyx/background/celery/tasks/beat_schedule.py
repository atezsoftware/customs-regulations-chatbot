...
# Add Elasticsearch migration task if enabled.
if (
    ENABLE_ELASTICSEARCH_INDEXING_FOR_ONYX
    and not DISABLE_ELASTICSEARCH_MIGRATION_TASK
    and not ONYX_DISABLE_VESPA
):
    beat_task_templates.append(
        {
            "name": "migrate-chunks-from-vespa-to-elasticsearch",
            "task": OnyxCeleryTask.MIGRATE_CHUNKS_FROM_VESPA_TO_ELASTICSEARCH_TASK,
            ...
            "options": {
                ...
                "expires": BEAT_EXPIRES_DEFAULT,
                "queue": OnyxCeleryQueues.ELASTICSEARCH_MIGRATION,
            },
        }
    )
...
_VECTOR_DB_BEAT_TASK_NAMES: set[str] = {
    ...
    "migrate-chunks-from-vespa-to-elasticsearch",
}
