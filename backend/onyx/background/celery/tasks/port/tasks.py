...
def run_port_attempt(port_attempt_id: int, celery_task_id: str | None = None) -> None:
    ...
    with get_session_with_current_tenant() as db_session:
        ...
        # A waiting reindex has a separate, swap-gated PostgreSQL -> FUTURE
        # projection for canonical regulatory files. Excluding those documents
        # here avoids running contextual enrichment twice and prevents the
        # create-only port from racing the canonical writer's replace operation.
        # INSTANT cannot use this optimization: after immediate promotion the
        # existing reconciler has no secondary target to supply.
        exclude_regulatory_user_files = (
            port_user_id is not None
            and future_search_settings.switchover_type != SwitchoverType.INSTANT
        )
    ...
    while True:
        ...
        with get_session_with_current_tenant() as db_session:
            ...
            if port_user_id is not None:
                doc_ids = get_user_file_ids_for_user_batch(
                    ...
                    exclude_regulatory_files=exclude_regulatory_user_files,
                )
            else:
        ...
        def _surviving_doc_ids(_ids: list[str] = doc_ids) -> set[str]:
            with get_session_with_current_tenant() as db_session:
                if port_user_id is not None:
                    return filter_existing_user_file_ids(
                        db_session,
                        port_user_id,
                        _ids,
                        exclude_regulatory_files=exclude_regulatory_user_files,
