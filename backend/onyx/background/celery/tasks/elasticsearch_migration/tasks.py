"""Celery tasks for migrating documents from Vespa to Elasticsearch."""
...
from onyx.background.celery.tasks.elasticsearch_migration.transformer import (
    transform_vespa_chunks_to_elasticsearch_chunks,
)
...
from onyx.db.elasticsearch_migration import (
    ...
    try_insert_elasticsearch_tenant_migration_record_with_commit,
...
@shared_task(
    name=OnyxCeleryTask.MIGRATE_CHUNKS_FROM_VESPA_TO_ELASTICSEARCH_TASK,
...
def migrate_chunks_from_vespa_to_elasticsearch_task(
    self: Task,  # noqa: ARG001
    *,
    tenant_id: str,
) -> bool | None:
    """
    Periodic task to migrate chunks from Vespa to Elasticsearch via the Visit API.

    Uses Vespa's Visit API to iterate through ALL chunks in bulk (not
    per-document), transform them, and index them into Elasticsearch. Progress is
    ...
    Returns:
        None if Elasticsearch migration is not enabled, or if the lock could not be
    ...
    # 1.a. If Elasticsearch indexing is disabled, we don't run the task.
    if not ENABLE_ELASTICSEARCH_INDEXING_FOR_ONYX:
        task_logger.warning(
            "Elasticsearch migration is not enabled, skipping chunk migration task."
        )
        return None
    task_logger.info("Starting chunk-level migration from Vespa to Elasticsearch.")
    ...
    lock: RedisLock = r.lock(
        name=OnyxRedisLocks.ELASTICSEARCH_MIGRATION_BEAT_LOCK,
    ...
    if not lock.acquire():
        task_logger.warning(
            "The Elasticsearch migration task timed out waiting for the lock."
        )
        return None
    else:
        task_logger.info(
            f"Acquired the Elasticsearch migration lock. Took {time.monotonic() - task_start_time:.3f} seconds. "
    ...
    try:
        ...
        if tenant_id != get_current_tenant_id():
            err_str = (
                f"Tenant ID mismatch in the Elasticsearch migration task: "
        ...
        with get_session_with_current_tenant() as db_session:
            ...
            # 2.c. Try to insert the ElasticsearchTenantMigrationRecord table if it
            # does not exist.
            ...
            if is_continuation_token_done_for_all_slices(continuation_token_map):
                task_logger.info(
                    f"Elasticsearch migration COMPLETED for tenant {tenant_id}. Total chunks migrated: {total_chunks_migrated}."
                )
        ...
        with get_vespa_http_client(
            ...
            # 2.g. Create the Elasticsearch and Vespa document indexes.
            ...
            while (
                ...
                # 3.b. Transform the raw chunks to Elasticsearch chunks in memory.
                elasticsearch_document_chunks, errored_chunks = (
                    transform_vespa_chunks_to_elasticsearch_chunks(
                        raw_vespa_chunks,
                        tenant_state,
                        sanitized_to_original_doc_id_mapping,
                ...
                if len(elasticsearch_document_chunks) != len(raw_vespa_chunks):
                    task_logger.error(
                        f"Migration task error: Number of candidate chunks to migrate ({len(elasticsearch_document_chunks)}) does "
                        f"not match number of chunks in Vespa ({len(raw_vespa_chunks)}). {len(errored_chunks)} chunks "
                        "errored."
                    )

                # 3.c. Index the Elasticsearch chunks into Elasticsearch.
                index_elasticsearch_chunks_start_time = time.monotonic()
                elasticsearch_document_index.index_raw_chunks(
                    chunks=elasticsearch_document_chunks
                )
                task_logger.debug(
                    f"Indexed {len(elasticsearch_document_chunks)} chunks into Elasticsearch in "
                    f"{time.monotonic() - index_elasticsearch_chunks_start_time:.3f} seconds."
                )

                total_chunks_migrated_this_task += len(elasticsearch_document_chunks)
                ...
                with get_session_with_current_tenant() as db_session:
                    ...
                    update_vespa_visit_progress_with_commit(
                        ...
                        chunks_processed=len(elasticsearch_document_chunks),
        ...
        traceback.print_exc()
        task_logger.exception("Error in the Elasticsearch migration task.")
        return False
    finally:
        if lock.owned():
            lock.release()
            task_logger.debug("Released the Elasticsearch migration lock.")
        else:
            task_logger.warning(
                "The Elasticsearch migration lock was not owned on completion of the migration task."
            )

    task_logger.info(
        f"Elasticsearch chunk migration task pausing (time limit reached). "
