...
from onyx.configs.constants import POSTGRES_CELERY_WORKER_REGULATORY_ANNEX_APP_NAME
...
celery_app.config_from_object("onyx.background.celery.configs.regulatory_annex")
...
def on_worker_init(sender: Worker, **kwargs: Any) -> None:
    ...
    SqlEngine.set_app_name(POSTGRES_CELERY_WORKER_REGULATORY_ANNEX_APP_NAME)
    pool_size = cast(int, sender.concurrency)  # ty: ignore[unresolved-attribute]
    SqlEngine.init_engine(pool_size=max(5, pool_size), max_overflow=2)
...
# These handlers are sibling modules, not autodiscovery's default tasks.py.
