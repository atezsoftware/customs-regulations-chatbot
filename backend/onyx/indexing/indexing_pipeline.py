...
from onyx.configs.app_configs import (
    CONTEXTUAL_RAG_LLM_TIMEOUT_SECONDS,
    CONTEXTUAL_RAG_MAX_RETRIES,
...
from onyx.db.models import IndexModelStatus, SearchSettings
...
# Celery can execute several indexing tasks concurrently in the same worker
# process. Their per-document pools share this gate so aggregate contextual LLM
# traffic does not multiply by the number of active documents.
_CONTEXTUAL_RAG_REQUEST_SLOTS = threading.BoundedSemaphore(CONTEXTUAL_RAG_MAX_WORKERS)


class ContextualEnrichmentError(RuntimeError):
    """A requested contextual enrichment could not be produced completely."""


def _contextual_prompt_content_budget(
    *,
    llm: LLM,
    tokenizer: BaseTokenizer,
    prompt_without_content: str,
    reserved_dynamic_tokens: int = 0,
) -> int:
    """Return a conservative token budget for dynamic contextual prompt text."""

    safe_input_limit = int(
        llm.config.max_input_tokens * (1 - GEN_AI_INPUT_TOKEN_SAFETY_MARGIN)
    )
    prompt_overhead = len(tokenizer.encode(prompt_without_content))
    content_budget = (
        safe_input_limit
        - prompt_overhead
        - reserved_dynamic_tokens
        - MAX_CONTEXT_TOKENS
    )
    if content_budget <= 0:
        raise ContextualEnrichmentError(
            "Contextual model input window is too small for the configured prompt "
            f"and output reserve: max_input_tokens={llm.config.max_input_tokens}"
        )
    return content_budget


def _trim_contextual_document(
    *,
    ...
    """Keep an in-budget document intact; otherwise retain its beginning and end."""

    if len(document_tokens) <= token_budget:
        return document_content
    try:
        return tokenizer_trim_middle(document_tokens, token_budget, tokenizer)
    except AssertionError as error:
        raise ContextualEnrichmentError(
            "Contextual document token budget is too small to preserve a "
            "representative document excerpt"
        ) from error


def _invoke_contextual_llm_with_retry(
    *,
    llm: LLM,
    prompt: LanguageModelInput,
    flow: LLMFlow,
) -> str:
    """Invoke a contextual LLM with bounded retry for transient provider errors."""

    attempts = CONTEXTUAL_RAG_MAX_RETRIES + 1
    for attempt in range(attempts):
        try:
            with llm_generation_span(
                ...
                with _CONTEXTUAL_RAG_REQUEST_SLOTS:
                    response = llm.invoke(
                        prompt,
                        timeout_override=CONTEXTUAL_RAG_LLM_TIMEOUT_SECONDS,
        ...
        except (LLMRateLimitError, LLMTimeoutError) as error:
            if attempt == attempts - 1:
                raise
            delay = min(
                CONTEXTUAL_RAG_RETRY_MAX_SECONDS,
                CONTEXTUAL_RAG_RETRY_BASE_SECONDS * (2**attempt),
            )
            logger.warning(
                "Transient contextual enrichment failure on attempt %d/%d; "
    ...
    raise RuntimeError("Contextual LLM retry loop exhausted without returning")
...
def add_document_summaries(
    ...
    *,
    ...
    doc_tokens = []
    # Callers pass only chunks with an individual contextual reserve. Keep the
    # guard for compatibility with direct callers.
    if chunks_by_doc[0].contextual_rag_reserved_tokens == 0:
        return None

    document_content = chunks_by_doc[0].source_document.get_text_content()
    doc_tokens = tokenizer.encode(document_content)
    doc_content = _trim_contextual_document(
    ...
    if not doc_summary.strip():
        message = (
            "Contextual document summarization returned an empty response for "
            f"document={chunks_by_doc[0].source_document.id}"
        )
        if raise_on_failure:
            raise ContextualEnrichmentError(message)
...
def add_chunk_summaries(
    ...
    *,
    ...
    """
    # Callers pass only chunks with an individual contextual reserve.
    if chunks_by_doc[0].contextual_rag_reserved_tokens == 0:
        return

    # use values computed in above doc summary section if available
    document_content = chunks_by_doc[0].source_document.get_text_content()
    doc_tokens = doc_tokens or tokenizer.encode(document_content)
    doc_content = _trim_contextual_document(
        document_content=document_content,
        document_tokens=doc_tokens,
    ...
    if not doc_info:
        ...
        if not doc_info.strip():
            message = (
                "Fallback contextual document summarization returned an empty "
                f"response for document={chunks_by_doc[0].source_document.id}"
            )
            if raise_on_failure:
                raise ContextualEnrichmentError(message)
            logger.warning(message)
    ...
    def assign_context(chunk: DocAwareChunk) -> None:
        ...
        try:
            ...
            if not chunk_context.strip():
                raise ContextualEnrichmentError(
                    "Contextual chunk summarization returned an empty response for "
                    f"document={chunk.source_document.id} chunk={chunk.chunk_id}"
                )
            chunk.chunk_context = chunk_context
        except Exception as error:
            chunk.chunk_context = ""
            if raise_on_failure:
                raise ContextualEnrichmentError(
                    "Contextual chunk enrichment failed for "
                    f"document={chunk.source_document.id} chunk={chunk.chunk_id}"
                ) from error
            logger.exception(
                "Error adding contextual chunk summary: %s",
                error,
                exc_info=error,
            )
    ...
    run_functions_tuples_in_parallel(
        ...
        max_workers=CONTEXTUAL_RAG_MAX_WORKERS,
    )
...
def add_contextual_summaries(
    ...
    *,
    ...
    # Keep tokenizer/provider drift and the requested output outside the dynamic
    # document budget. This is especially important when the configured model
    # tokenizer is unavailable and token counting falls back to the embedding
    # tokenizer.
    trunc_doc_summary_tokens = _contextual_prompt_content_budget(
        llm=llm,
        tokenizer=tokenizer,
    ...
    trunc_doc_chunk_tokens = _contextual_prompt_content_budget(
        llm=llm,
        tokenizer=tokenizer,
        prompt_without_content=(
            CONTEXTUAL_RAG_PROMPT1.format(document="")
            + CONTEXTUAL_RAG_PROMPT2.format(chunk="")
        ),
        reserved_dynamic_tokens=chunk_token_limit,
    )
    for chunks_by_doc in doc2chunks.values():
        eligible_chunks = [
            chunk for chunk in chunks_by_doc if chunk.contextual_rag_reserved_tokens > 0
        ]
...
def run_indexing_pipeline(
    *,
    ...
    index_to_secondary: bool = False,
    from_beginning: bool = False,
    search_settings_override: SearchSettings | None = None,
) -> IndexingPipelineResult:
    ...
    """
    if search_settings_override is not None:
        search_settings = search_settings_override
    else:
