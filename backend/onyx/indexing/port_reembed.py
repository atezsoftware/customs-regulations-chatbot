...
def _reconstruct_source_document(
    ...
    *,
    ...
    return Document(
        id=document_id or first.document_id,
...
def _augmentation_reembed(
    ...
    contextual_source_documents: dict[tuple[str, int], tuple[Document, bool]] = {}
    today = datetime.date.today()
    for doc_id, pairs in pairs_by_doc.items():
        regulatory_pairs = [
            pair for pair in pairs if pair[0].regulatory_chunk_id is not None
        ]
        snapshot_cache: dict[tuple[str, ...], Document] = {}
        for stored, _bare in pairs:
            identity = (stored.document_id, stored.chunk_index)
            if stored.regulatory_chunk_id is None:
                contextual_source_documents[identity] = (
                    canonical_source_documents[doc_id],
                    True,
                )
                continue

            reference_date = context_reference_date(
                stored.validity_start_date,
                stored.validity_end_date,
                today=today,
            )
            visible_pairs = [
                pair
                ...
                if validity_window_contains(
                    pair[0].validity_start_date,
                    pair[0].validity_end_date,
                    reference_date,
                )
            ]
            if all(
                pair[0].regulatory_chunk_id != stored.regulatory_chunk_id
                for pair in visible_pairs
            ):
                visible_pairs.append((stored, _bare))
            visible_pairs.sort(key=lambda pair: pair[0].chunk_index)
            ...
            snapshot_key = tuple(
                pair[0].regulatory_chunk_id or "" for pair in visible_pairs
            )
            snapshot_document = snapshot_cache.get(snapshot_key)
            if snapshot_document is None:
                snapshot_digest = hashlib.sha256(
                    "|".join(snapshot_key).encode()
                ).hexdigest()[:12]
                snapshot_chunks = [pair[0] for pair in visible_pairs]
                snapshot_contents = [
                    (
                        f"{' > '.join(pair[0].heading_path)}\n{pair[1]}"
                        if pair[0].heading_path
                        else pair[1]
                    )
                    for pair in visible_pairs
                ]
                snapshot_document = _reconstruct_source_document(
                    snapshot_chunks,
                    snapshot_contents,
                    document_id=(
                        f"{doc_id}::regulatory-context::"
                        f"{reference_date.isoformat()}::{snapshot_digest}"
                    ),
                )
                snapshot_cache[snapshot_key] = snapshot_document
            contextual_source_documents[identity] = (snapshot_document, True)
    ...
    for chunk, bare in zip(stored_chunks, bare_contents):
        ...
        semantic_tail = _future_semantic_tail(chunk, ctx.future_embedding_tokenizer)
        context_document, has_useful_context = contextual_source_documents[
            (chunk.document_id, chunk.chunk_index)
        ]
        reserved = ctx.contextual_rag_reserved_tokens if future_rag_on else 0
        if future_rag_on and chunk.regulatory_chunk_id is not None:
            reserved = (
                contextual_reserve_for_embedding_text(
                    f"{title_prefix}{bare}{semantic_tail}",
                    tokenizer=ctx.future_embedding_tokenizer,
                    embedding_token_limit=chunk.max_chunk_size,
                    requested_reserve=ctx.contextual_rag_reserved_tokens,
                )
                if has_useful_context
                else 0
            )
        doc_aware_chunks.append(
            DocAwareChunk(
                chunk_id=chunk.chunk_index,
                blurb=chunk.blurb,
                content=bare,
                ...
                source_document=(
                    context_document
                    if reserved > 0
                    else canonical_source_documents[chunk.document_id]
                ),
                title_prefix=title_prefix,
                metadata_suffix_semantic=semantic_tail,
    ...
    for stored, chunk in zip(stored_chunks, doc_aware_chunks):
        chunk.source_document = canonical_source_documents[stored.document_id]
