...
def cleanup_content_for_chunks(
    ...
    def _remove_contextual_rag(chunk: InferenceChunkUncleaned) -> str:
        # The combined index content is Unicode-sanitized before storage. Apply
        # that same normalization to the separately stored inverse fields so a
        # control character cannot make the prefix/suffix comparison fail and
        # expose generated retrieval context as document evidence.
        doc_summary = remove_invalid_unicode_chars(chunk.doc_summary)
        chunk_context = remove_invalid_unicode_chars(chunk.chunk_context)
        # remove document summary
        if doc_summary and chunk.content.startswith(doc_summary):
            chunk.content = chunk.content[len(doc_summary) :].lstrip()
        # remove chunk context
        if chunk_context and chunk.content.endswith(chunk_context):
            chunk.content = chunk.content[: -len(chunk_context)].rstrip()
        return chunk.content
