...
class OpenSearchIndexClient(OpenSearchClient):
    ...
    def validate_regulatory_chunk_projection(
        ...
        *,
        ...
        ``expected_chunks`` maps the deterministic OpenSearch document id to
        ...
        if not isinstance(documents, list):
            raise OpenSearchUpdateError(
                "OpenSearch returned a malformed regulatory projection preflight."
            )
        ...
        for document in documents:
            if not isinstance(document, dict):
                raise OpenSearchUpdateError(
                    "OpenSearch returned a malformed document in regulatory "
                    "projection preflight."
                )
            chunk_id = document.get("_id")
            if not isinstance(chunk_id, str) or chunk_id not in expected_chunks:
                raise OpenSearchUpdateError(
                    "OpenSearch returned an unexpected chunk in regulatory "
                    "projection preflight."
                )
            seen.add(chunk_id)
        ...
        if missing:
            raise OpenSearchDocumentMissingError(sorted(set(missing)))
        if mismatches:
