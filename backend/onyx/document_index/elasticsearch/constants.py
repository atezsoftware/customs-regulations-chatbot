...
# By default Elasticsearch will only return a maximum of this many results in a
...
class HybridSearchNormalizationMethod(Enum):
    # Current default.
    MIN_MAX = 1
...
# Will raise and block application start if HYBRID_SEARCH_NORMALIZATION_METHOD
# is set but not a valid value. If not set, defaults to MIN_MAX.
HYBRID_SEARCH_NORMALIZATION_METHOD: HybridSearchNormalizationMethod = (
    HybridSearchNormalizationMethod(
        int(os.environ["HYBRID_SEARCH_NORMALIZATION_METHOD"])
    )
    if os.environ.get("HYBRID_SEARCH_NORMALIZATION_METHOD", None) is not None
    else HybridSearchNormalizationMethod.MIN_MAX
)
