...
from onyx.configs.app_configs import (
    ELASTICSEARCH_INDEX_NUM_REPLICAS,
    ELASTICSEARCH_INDEX_NUM_SHARDS,
...
from onyx.document_index.elasticsearch.string_filtering import (
...
# Regulatory chunk projection fields — set only for chunks produced by the
# RegulatoryChunker over user files (see backend/onyx/regulatory/). Postgres
...
def get_elasticsearch_doc_chunk_id(
    ...
    This will be the string used to identify the chunk in Elasticsearch. Any direct
    chunk queries should use this function.

    If the document ID is too long, a hash of the ID is used instead.
    """
    elasticsearch_doc_chunk_id_suffix: str = f"__{max_chunk_size}__{chunk_index}"
    encoded_suffix_length: int = len(elasticsearch_doc_chunk_id_suffix.encode("utf-8"))
    max_encoded_permissible_doc_id_length: int = (
        MAX_DOCUMENT_ID_ENCODED_LENGTH - encoded_suffix_length
    )
    elasticsearch_doc_chunk_id_tenant_prefix: str = ""
    if tenant_state.multitenant:
        ...
        elasticsearch_doc_chunk_id_tenant_prefix = f"{short_tenant_id}__"
        encoded_prefix_length: int = len(
            elasticsearch_doc_chunk_id_tenant_prefix.encode("utf-8")
        )
    ...
    elasticsearch_doc_chunk_id: str = f"{elasticsearch_doc_chunk_id_tenant_prefix}{sanitized_document_id}{elasticsearch_doc_chunk_id_suffix}"

    # Do one more validation to ensure we haven't exceeded the max length.
    elasticsearch_doc_chunk_id = filter_and_validate_document_id(
        elasticsearch_doc_chunk_id
    )
    return elasticsearch_doc_chunk_id
...
class DocumentChunkWithoutVectors(BaseModel):
    ...
    The names of these fields are based on the Elasticsearch schema. Changes to the
    ...
    # Regulatory chunk projection (None for non-regulatory chunks).
    # regulatory_chunk_id targets updates/deletes of a specific Postgres row's
    # projection; heading_path locates the chunk inside its document (used for
    # citations and full-text matching); the validity window backs as-of-date
    ...
    def parse_epoch_seconds_to_datetime(
        ...
        if not isinstance(value, int):
            raise ValueError(
                f"Bug: Expected an int for the datetime property '{info.field_name}' from Elasticsearch, got {type(value)} instead."
            )
    ...
    def parse_tenant_id(cls, value: Any) -> TenantState:
        """
        Generates a TenantState from Elasticsearch's tenant_id if it exists, or
        ...
        if value is None:
            ...
            raise ValueError(
                f"Bug: Expected a str for the tenant_id property from Elasticsearch, got {type(value)} instead."
            )
        else:
            if not MULTI_TENANT:
                raise ValueError(
                    "Bug: Got a non-null str for the tenant_id property from Elasticsearch but "
...
class DocumentChunk(DocumentChunkWithoutVectors):
    """Represents a chunk of a document in the Elasticsearch index.
...
class DocumentSchema:
    """
    Represents the schema and indexing strategies of the Elasticsearch index.
    ...
    def get_document_schema(vector_dimension: int, multitenant: bool) -> dict[str, Any]:
        """Returns the document schema for the Elasticsearch index.
        ...
        Notes:
          ...
          - "type": "keyword" fields are stored as-is, used for exact matches,
            filtering, etc.
          - "type": "text" fields are Elasticsearch-processed strings, used for
            full-text searches.
        ...
        Returns:
            A dictionary representing the document schema, to be supplied to the
                Elasticsearch client. The structure of this dictionary is
                determined by Elasticsearch documentation.
        """
        schema: dict[str, Any] = {
            # By default Elasticsearch allows dynamically adding new properties
            ...
            "properties": {
                TITLE_FIELD_NAME: {
                    ...
                    # ELASTICSEARCH_TEXT_ANALYZER. Existing indices need reindexing
                    # after a change.
                ...
                # Elasticsearch's terms query with value_type: "bitmap" can
                # efficiently check if any value in this array matches a
                # query bitmap.
                ANCESTOR_HIERARCHY_NODE_IDS_FIELD_NAME: {"type": "integer"},
                # Regulatory chunk projection fields; absent on non-regulatory
                # chunks. See the DocumentChunkWithoutVectors field comments.
                REGULATORY_CHUNK_ID_FIELD_NAME: {"type": "keyword"},
    ...
    def get_index_settings_based_on_environment() -> dict[str, Any]:
        ...
        if ELASTICSEARCH_INDEX_NUM_SHARDS is not None:
            number_of_shards = ELASTICSEARCH_INDEX_NUM_SHARDS
        if ELASTICSEARCH_INDEX_NUM_REPLICAS is not None:
            number_of_replicas = ELASTICSEARCH_INDEX_NUM_REPLICAS
