...
_SEMANTIC_QUERY_REPHRASE_MAX_TOKENS = 512
_KEYWORD_QUERY_EXPANSION_MAX_TOKENS = 256
...
def semantic_query_rephrase(
    ...
    with llm_generation_span(
        ...
        response = llm.invoke(
            ...
            max_tokens=_SEMANTIC_QUERY_REPHRASE_MAX_TOKENS,
        )
...
def keyword_query_expansion(
    ...
    with llm_generation_span(
        ...
        response = llm.invoke(
            prompt=messages,
            reasoning_effort=ReasoningEffort.OFF,
            max_tokens=_KEYWORD_QUERY_EXPANSION_MAX_TOKENS,
        )
        record_llm_response(span_generation, response)
