...
class BufferedEmitter(Emitter):
    """Hold untagged packets until a chat step is safe to publish.

    Replaying through the destination emitter preserves the canonical model-index
    tagging and disconnect handling. A discarded buffer has no streaming side
    effects.
    """
    ...
    def replay_to(self, emitter: Emitter) -> None:
