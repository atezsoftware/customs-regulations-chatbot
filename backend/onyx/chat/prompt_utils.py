...
from onyx.prompts.regulatory_guidance import REGULATORY_ANALYSIS_GUIDANCE
...
def append_grounding_guidance(system_prompt: str) -> str:
    """Enforce the grounding rules on whichever prompt ends up as the system message.

    Applied outside the editable prompt text so an admin-edited default prompt or a
    custom agent prompt cannot drop it.
    """
    guidance_blocks = (GROUNDING_GUIDANCE, REGULATORY_ANALYSIS_GUIDANCE)
    missing_blocks = [
        guidance
        for guidance in guidance_blocks
        if guidance.strip() not in system_prompt
    ]
...
def build_system_prompt(
    ...
    if include_all_guidance:
        ...
        return append_grounding_guidance(system_prompt)
