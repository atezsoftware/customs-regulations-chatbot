...
from onyx.tools.constants import (
    FILE_READER_TOOL_ID,
...
def _filter_global_regulatory_chat_tools(
    model_tools: list[Tool], *, is_global_regulatory_chat: bool
) -> list[Tool]:
    """Regulatory chats may retrieve only through the chunk SearchTool.

    FileReaderTool bypasses the regulatory chunk index and can expose an
    entire file. Remove it even if a deployment configuration makes
    that tool available in the future.
    """
...
def extract_context_files(
    ...
    Args:
        ...
        force_search_only: Skip the fit check and always route through search.
            The global regulatory chat uses this to keep retrieval in the chunk
            index rather than injecting whole files into the prompt.
    Returns:
    ...
    if force_search_only or aggregate_tokens >= max_actual_tokens:
...
def determine_search_params(
    ...
    """Resolve search scope for custom agents and the global regulatory chat.

    Custom agents retain their own persona-scoped file behavior. The default
    chat always gets SearchTool with no project filter: Projects organize chat
    sessions, while the shared Files corpus defines the knowledge scope.
    """
    ...
    if is_custom_persona:
        return SearchParams(
            ...
            persona_id_filter=(
                persona_id if extracted_context_files.use_as_search_filter else None
            ),
...
def _should_auto_detect_search_filters(
    *, persona_id: int, workspace_setting_enabled: bool
    ...
    """The global regulatory scope is already fixed to current user-file chunks."""
    return workspace_setting_enabled and persona_id != DEFAULT_PERSONA_ID
...
def build_chat_turn(
    ...
    *,
    ...
    if (
        ...
        and Permission.FULL_ADMIN_PANEL_ACCESS not in get_effective_permissions(user)
    ):
        raise OnyxError(
            OnyxErrorCode.INSUFFICIENT_PERMISSIONS,
            "Only administrators can attach files to chats.",
        )

    # Verify that any admin-specified files actually belong to the user.
    verify_user_files(
    ...
    if is_global_regulatory_chat:
        # The shared corpus is searchable through indexed chunks only. Do not
        # hand file IDs to any direct-file tool.
        available_files = AvailableFiles(user_file_ids=[], chat_file_ids=[])
    ...
    # Project files do not define the default chat's knowledge boundary. Custom
    # agents retain their own persona-scoped file behavior.
    context_user_files = (
        []
        if is_global_regulatory_chat
        else resolve_context_user_files(
            persona=persona,
            project_id=chat_session.project_id,
            user_id=user_id,
            db_session=db_session,
    ...
    # The default chat always retrieves through the regulatory chunk index.
    extracted_context_files = extract_context_files(
        ...
        db_session=db_session,
        force_search_only=is_global_regulatory_chat,
    )
