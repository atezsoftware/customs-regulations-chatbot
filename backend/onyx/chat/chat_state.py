...
# A saved search result is one retrieved chunk. Highlights can vary between
# queries without changing the legal source identity.
SearchDocKey = tuple[str, int]
...
class ChatStateContainer:
    ...
    @staticmethod
    def create_search_doc_key(search_doc: SearchDoc) -> SearchDocKey:
        """Create a unique key for a SearchDoc for deduplication.

        Args:
            search_doc: The SearchDoc to create a key for
        """
        return (search_doc.document_id, search_doc.chunk_ind)
    ...
    def add_search_docs(self, search_docs: list[SearchDoc]) -> None:
        ...
        with self._lock:
            for doc in search_docs:
                key = self.create_search_doc_key(doc)
