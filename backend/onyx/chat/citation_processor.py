...
class DynamicCitationProcessor:
    ...
    def fork(self) -> "DynamicCitationProcessor":
        """Return an isolated copy for a response that may be discarded.

        Search documents are immutable for the duration of a chat step, so the
        mappings can be shallow-copied. Every mutable parser and citation-tracking
        collection is copied to prevent a rejected draft from suppressing or
        renumbering citations in a later accepted response.
        """

        forked = DynamicCitationProcessor(
            citation_mode=self.citation_mode,
            stop_stream=self.stop_stream,
        )
        forked.citation_to_doc = self.citation_to_doc.copy()
        forked.seen_citations = self.seen_citations.copy()
        forked.llm_out = self.llm_out
        forked.curr_segment = self.curr_segment
        forked.hold = self.hold
        forked.cited_documents_in_order = self.cited_documents_in_order.copy()
        forked.cited_document_ids = self.cited_document_ids.copy()
        forked.recent_cited_documents = self.recent_cited_documents.copy()
        forked.non_citation_count = self.non_citation_count
        return forked
    ...
    def _process_citation(
        ...
        for num_str in citation_content.split(","):
            ...
            citation_identity = f"{doc_id}:{search_doc.chunk_ind}"
            ...
            if citation_identity in self.recent_cited_documents:
                continue
            self.recent_cited_documents.add(citation_identity)

            # Different chunks of the same document are distinct legal citations.
            if citation_identity not in self.cited_document_ids:
                self.cited_document_ids.add(citation_identity)
