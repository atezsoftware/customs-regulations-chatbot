#!/usr/bin/env python3
"""Benchmarks ElasticsearchDocumentIndex latency.

Requires Onyx to be running as it reads search settings from the database.

Usage:
    source .venv/bin/activate
    python backend/scripts/debugging/elasticsearch/benchmark_retrieval.py --help
"""
...
from scripts.debugging.elasticsearch.constants import DEV_TENANT_ID
from scripts.debugging.elasticsearch.embedding_io import load_query_embedding_from_file
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="A benchmarking tool to measure Elasticsearch retrieval latency."
    )
    parser.add_argument(
        "-n",
        type=int,
