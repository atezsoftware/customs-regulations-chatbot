#!/usr/bin/env python3
"""A utility to interact with Elasticsearch.

Example Usage:
    Assuming running from ~/onyx/
        source .venv/bin/activate
        python backend/scripts/debugging/elasticsearch/elasticsearch_debug.py --help
        python backend/scripts/debugging/elasticsearch/elasticsearch_debug.py list
        python backend/scripts/debugging/elasticsearch/elasticsearch_debug.py delete
            <index_name>

Environment Variables:
    ELASTICSEARCH_HOST: Elasticsearch host
    ELASTICSEARCH_REST_API_PORT: Elasticsearch port
    ELASTICSEARCH_ADMIN_USERNAME: Admin username
    ELASTICSEARCH_ADMIN_PASSWORD: Admin password

Dependencies:
    backend/shared_configs/configs.py
    backend/onyx/document_index/elasticsearch/client.py
"""
...
def list_indices(client: ElasticsearchClient) -> None:
...
def delete_index(client: ElasticsearchIndexClient) -> None:
...
def set_settings(client: ElasticsearchIndexClient, settings: dict[str, Any]) -> None:
    client.update_settings(settings)
    print(f"Updated settings for index '{client._index_name}'.")


def open_index(client: ElasticsearchIndexClient) -> None:
    client.open_index()
    print(f"Index '{client._index_name}' opened.")


def close_index(client: ElasticsearchIndexClient) -> None:
    client.close_index()
    print(f"Index '{client._index_name}' closed.")


def reroute_retry_failed(client: ElasticsearchClient) -> None:
    print(
...
def diagnose_health(client: ElasticsearchClient) -> None:
...
def main() -> None:
    def add_standard_arguments(parser: argparse.ArgumentParser) -> None:
        parser.add_argument(
            "--host",
            help="Elasticsearch host. If not provided, will fall back to ELASTICSEARCH_HOST, then prompt for input.",
            type=str,
            default=os.environ.get("ELASTICSEARCH_HOST", ""),
        )
        parser.add_argument(
            "--port",
            help="Elasticsearch port. If not provided, will fall back to ELASTICSEARCH_REST_API_PORT, then prompt for input.",
            type=int,
            default=int(os.environ.get("ELASTICSEARCH_REST_API_PORT", 0)),
        )
        parser.add_argument(
            "--username",
            help=(
                "Elasticsearch username. If not provided, will fall back to ELASTICSEARCH_ADMIN_USERNAME, then prompt for "
                "input."
            ),
            type=str,
            default=os.environ.get("ELASTICSEARCH_ADMIN_USERNAME", ""),
        )
        parser.add_argument(
            "--password",
            help=(
                "Elasticsearch password. If not provided, will fall back to ELASTICSEARCH_ADMIN_PASSWORD, then prompt for "
                "input."
            ),
            type=str,
            default=os.environ.get("ELASTICSEARCH_ADMIN_PASSWORD", ""),
        )
        parser.add_argument(
    ...
    parser = argparse.ArgumentParser(
        description="A utility to interact with Elasticsearch."
    )
    ...
    if not (host := args.host or input("Enter the Elasticsearch host: ")):
        print("Error: Elasticsearch host is required.")
        sys.exit(1)
    if not (port := args.port or int(input("Enter the Elasticsearch port: "))):
        print("Error: Elasticsearch port is required.")
        sys.exit(1)

    if not (username := args.username or input("Enter the Elasticsearch username: ")):
        print("Error: Elasticsearch username is required.")
        sys.exit(1)
    if not (password := args.password or input("Enter the Elasticsearch password: ")):
        print("Error: Elasticsearch password is required.")
        sys.exit(1)
    ...
    with (
        ElasticsearchClient(**common_kwargs)
        if args.command in cluster_only_commands
        else ElasticsearchIndexClient(index_name=args.index, **common_kwargs)
    ) as client:
        if not client.ping():
