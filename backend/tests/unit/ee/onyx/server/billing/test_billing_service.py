...
class TestMakeBillingRequest:
    ...
    @pytest.mark.asyncio
    async def test_blocks_request_when_vendor_upstream_is_disabled(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        from ee.onyx.server.billing import service

        monkeypatch.setattr(service, "DISABLE_ONYX_UPSTREAM_CONNECTIONS", True)

        with patch("httpx.AsyncClient") as mock_client:
            with pytest.raises(OnyxError) as exc_info:
                await service._make_billing_request(method="GET", path="/test")
