...
def test_time_scope_decision_bounds_llm_output() -> None:
    history = [
        ChatMinimalTextMessage(
            message="What changed last January?", message_type=MessageType.USER
        )
    ...
    response = MagicMock()
    response.choice.message.content = "updated (2026-01-01, 2026-01-31)"
    llm = MagicMock()
    ...
    with (
        ...
        assert decide_time_filter(history, llm) is not None

    assert llm.invoke.call_args.kwargs["max_tokens"] == 64
