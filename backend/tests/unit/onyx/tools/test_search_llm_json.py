...
class TestCodeInterpreterFilenameInLLMJson:
    def test_filename_embeds_file_id_when_present(self) -> None:
        ...
        llm_string, _, _ = convert_inference_sections_to_llm_string([section])
    ...
    def test_same_title_distinct_file_ids_produce_distinct_names(self) -> None:
        ...
        llm_string, _, _ = convert_inference_sections_to_llm_string(
    ...
    def test_citation_mapping_unchanged_by_file_presence(self) -> None:
        ...
        _, citation_mapping, citation_chunk_mapping = (
            convert_inference_sections_to_llm_string([section], citation_start=42)
        )

        assert citation_mapping == {42: "doc-e"}
        assert citation_chunk_mapping == {42: 0}

    def test_same_document_chunks_receive_distinct_citations(self) -> None:
        first = _make_chunk("doc-shared", chunk_id=7)
        second = _make_chunk("doc-shared", chunk_id=11)

        llm_string, citation_mapping, citation_chunk_mapping = (
            convert_inference_sections_to_llm_string(
                [_make_section(first), _make_section(second)], citation_start=20
            )
        )

        results = json.loads(llm_string)["results"]
        assert [result["document"] for result in results] == [20, 21]
        assert citation_mapping == {20: "doc-shared", 21: "doc-shared"}
        assert citation_chunk_mapping == {20: 7, 21: 11}
...
class TestContentFieldWrappingForFileBearingHits:
    ...
    def test_regulatory_hit_stays_chunk_scoped_even_when_file_id_exists(self) -> None:
        chunk = _make_chunk(
            "doc-regulatory",
            semantic_identifier="Kanun — MADDE 10 > (2)",
            file_id="file-regulatory",
            content="only the matched paragraph",
            regulatory_chunk_id="rc_exact",
        )
        section = _make_section(
            chunk,
            combined_content="matched paragraph and bounded provision siblings",
        )

        llm_string, _, _ = convert_inference_sections_to_llm_string([section])
        result = json.loads(llm_string)["results"][0]

        assert result["content"] == ("matched paragraph and bounded provision siblings")
        assert "file_name" not in result
        assert "code interpreter" not in result["content"].lower()

    def test_regulatory_hit_exposes_canonical_projection_metadata(self) -> None:
        chunk = _make_chunk(
            "doc-regulatory",
            regulatory_chunk_id="rc-10-2",
            heading_path=["Kanun", "MADDE 10", "(2)"],
        ...
        assert json.loads(result["metadata"]) == {
            "jurisdiction": "TR",
            "regulatory_chunk_id": "rc-10-2",
            "regulatory_heading_path": ["Kanun", "MADDE 10", "(2)"],
        }

    def test_generic_hit_does_not_gain_regulatory_metadata(self) -> None:
        llm_string, _, _ = convert_inference_sections_to_llm_string(
            [_make_section(_make_chunk("generic"))]
        )

        assert "metadata" not in json.loads(llm_string)["results"][0]

    def test_generic_hit_preserves_existing_metadata_without_regulatory_fields(
        self,
    ) -> None:
        chunk = _make_chunk(
            "generic",
            metadata={"department": "operations", "tags": ["current"]},
        )
        ...
        assert json.loads(json.loads(llm_string)["results"][0]["metadata"]) == {
    ...
    def test_mixed_batch_only_file_hits_get_guidance(self) -> None:
        ...
        llm_string, _, _ = convert_inference_sections_to_llm_string(sections)
        results = json.loads(llm_string)["results"]
