...
def test_fork_isolates_all_mutable_citation_state(
    mock_search_docs: CitationMapping,
) -> None:
    processor = DynamicCitationProcessor()
    processor.update_citation_mapping({1: mock_search_docs[1]})
    process_tokens(processor, ["Supported [1]."])

    forked = processor.fork()
    forked.update_citation_mapping({2: mock_search_docs[2]})
    process_tokens(forked, ["Additional [2]."])
    forked.reset_recent_citations()

    assert 2 not in processor.citation_to_doc
    assert 2 not in processor.get_seen_citations()
    assert processor.cited_documents_in_order == [mock_search_docs[1]]
    assert forked.cited_documents_in_order == [
        mock_search_docs[1],
        mock_search_docs[2],
    ]
    assert processor.recent_cited_documents != forked.recent_cited_documents
...
def test_different_chunks_of_same_document_emit_distinct_citations() -> None:
    ...
    _, citations = process_tokens(processor, ["Rules [1] and [2]."])

    assert [(item.document_id, item.chunk_ind) for item in citations] == [
        ("shared-doc", 7),
        ("shared-doc", 12),
    ]
