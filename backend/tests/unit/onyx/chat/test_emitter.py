...
class TestBufferedEmitter:
    def test_replay_preserves_order_and_uses_destination_tagging(self) -> None:
        destination, merged_queue = _make_emitter(model_idx=2)
        buffered = BufferedEmitter()
        buffered.emit(_packet(turn_index=3))
        buffered.emit(_packet(turn_index=4))

        assert merged_queue.empty()
        buffered.replay_to(destination)

        first_key, first_packet = merged_queue.get_nowait()
        second_key, second_packet = merged_queue.get_nowait()
        assert (first_key, second_key) == (2, 2)
        assert first_packet.placement.turn_index == 3
        assert second_packet.placement.turn_index == 4
        assert first_packet.placement.model_index == 2
        assert second_packet.placement.model_index == 2

    def test_discarding_buffer_has_no_destination_side_effect(self) -> None:
        _destination, merged_queue = _make_emitter()
        buffered = BufferedEmitter()
        buffered.emit(_packet())

        assert len(buffered.get_packets()) == 1
        assert merged_queue.empty()
