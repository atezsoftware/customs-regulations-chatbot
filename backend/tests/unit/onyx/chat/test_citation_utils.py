...
def test_tool_response_maps_citation_to_exact_chunk() -> None:
    first = create_test_search_doc(document_id="shared", chunk_ind=3)
    second = create_test_search_doc(document_id="shared", chunk_ind=9)
    response = ToolResponse(
        rich_response=SearchDocsResponse(
            search_docs=[first, second],
            citation_mapping={41: "shared", 42: "shared"},
            citation_chunk_mapping={41: 3, 42: 9},
        ),
        llm_facing_response="",
    ...
    assert processor.citation_to_doc[41].chunk_ind == 3
    assert processor.citation_to_doc[42].chunk_ind == 9


def test_search_response_reuses_canonical_number_for_exact_chunk() -> None:
    existing = create_test_search_doc(document_id="shared", chunk_ind=3)
    repeated = create_test_search_doc(document_id="shared", chunk_ind=3)
    new = create_test_search_doc(document_id="new", chunk_ind=8)
    response = ToolResponse(
        rich_response=SearchDocsResponse(
            search_docs=[repeated, new],
            citation_mapping={105: "shared", 106: "new"},
            citation_chunk_mapping={105: 3, 106: 8},
        ),
        llm_facing_response=(
            '{"receipt": {}, "results": [{"document": 105}, {"document": 106}]}'
        ),
        tool_call=ToolCallKickoff(
            tool_args={},
            tool_name="internal_search",
...
def test_chat_state_preserves_distinct_chunks_from_same_document() -> None:
    ...
    state.add_search_docs([first, second])

    assert set(state.get_all_search_docs()) == {("shared", 3), ("shared", 9)}
...
class TestCollapseCitationsMultipleCitations:
    ...
    def test_same_document_different_chunks_keep_distinct_numbers(self) -> None:
        new_mapping: CitationMapping = {
            50: create_test_search_doc(document_id="same_doc", chunk_ind=4),
            60: create_test_search_doc(document_id="same_doc", chunk_ind=9),
        }

        text, mapping = collapse_citations("[50] and [60]", {}, new_mapping)

        assert text == "[1] and [2]"
        assert mapping[1].chunk_ind == 4
        assert mapping[2].chunk_ind == 9
