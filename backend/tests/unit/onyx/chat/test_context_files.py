...
def test_global_regulatory_chat_tools_remove_direct_file_reader() -> None:
    ...
    search_tool.name = "run_search"
    file_reader_tool = MagicMock(name="file_reader_tool")
    file_reader_tool.name = "read_file"

    filtered = _filter_global_regulatory_chat_tools(
        [search_tool, file_reader_tool], is_global_regulatory_chat=True
    )

    assert filtered == [search_tool]


def test_custom_agent_tools_keep_file_reader() -> None:
    file_reader_tool = MagicMock(name="file_reader_tool")
    file_reader_tool.name = "read_file"

    assert _filter_global_regulatory_chat_tools(
        [file_reader_tool], is_global_regulatory_chat=False
    ) == [file_reader_tool]
...
def test_custom_agent_search_filters_are_preserved() -> None:
    expected = BaseFilters(source_type=[DocumentSource.WEB])
    setup = MagicMock()
    setup.persona.id = 42
...
def test_global_regulatory_chat_skips_redundant_filter_llm_detection() -> None:
    assert not _should_auto_detect_search_filters(
        persona_id=DEFAULT_PERSONA_ID,
        workspace_setting_enabled=True,
    )
    assert _should_auto_detect_search_filters(
        persona_id=42,
        workspace_setting_enabled=True,
    )
    assert not _should_auto_detect_search_filters(
        persona_id=42,
        workspace_setting_enabled=False,
    )
...
class TestSearchFilterDetermination:
    ...
    def test_default_persona_project_files_fit_uses_global_search(self) -> None:
        """Default persona ignores project files and uses global search."""
        result = determine_search_params(
            persona_id=DEFAULT_PERSONA_ID,
            extracted_context_files=self._make_context(
                file_texts=["content"],
    ...
    def test_default_persona_project_files_overflow_uses_global_search(self) -> None:
        """Default persona never narrows global search to the project."""
        result = determine_search_params(
            persona_id=DEFAULT_PERSONA_ID,
            extracted_context_files=self._make_context(
    ...
    def test_default_persona_no_project_enables_global_search(self) -> None:
        """Default persona searches the shared corpus without a project."""
        result = determine_search_params(
            persona_id=DEFAULT_PERSONA_ID,
            extracted_context_files=self._make_context(),
        )
    ...
    def test_default_persona_project_no_files_enables_global_search(self) -> None:
        """An empty personal project still searches the shared corpus."""
        result = determine_search_params(
            persona_id=DEFAULT_PERSONA_ID,
            extracted_context_files=self._make_context(),
        )
