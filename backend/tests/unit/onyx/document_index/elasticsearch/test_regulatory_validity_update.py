...
def _document_index() -> tuple[ElasticsearchDocumentIndex, MagicMock, TenantState]:
...
def test_regulatory_validity_patch_preflights_exact_ids_and_updates_metadata() -> None:
    ...
    chunk_ids = [
        get_elasticsearch_doc_chunk_id(tenant_state, "doc-1", chunk_index)
...
def test_regulatory_validity_preflight_failure_writes_nothing() -> None:
    index, client, _ = _document_index()
    client.validate_regulatory_chunk_projection.side_effect = ElasticsearchUpdateError(
        "mismatch"
    )

    with pytest.raises(ElasticsearchUpdateError, match="mismatch"):
        index.update_regulatory_validity(
            document_id="doc-1",
            expected_regulatory_chunk_ids=["rc-a"],
            previous_start_date=None,
...
def _index_client() -> tuple[ElasticsearchIndexClient, MagicMock]:
...
def test_projection_preflight_rejects_stale_extra_chunk_before_identity_read() -> None:
    ...
    with pytest.raises(ElasticsearchUpdateError, match="chunk count mismatch"):
        client.validate_regulatory_chunk_projection(
            {"os-0": ("doc-1", 0, "rc-a")},
            validity_start_date=None,
            validity_end_date=None,
