...
Pure-Python seams:
- `ElasticsearchDocumentIndex.update` keeps processing requests past a missing-doc
  one and reports only the docs that were actually missing (chunk -> doc mapping).
- `ElasticsearchIndexPair.update` lets PRESENT (primary) write, then converts a
  secondary `ElasticsearchDocumentMissingError` into a typed
...
tests/external_dependency_unit/elasticsearch/test_elasticsearch_client.py.
"""
...
) -> tuple[ElasticsearchIndexPair, MagicMock]:
...
def _make_index() -> tuple[ElasticsearchDocumentIndex, TenantState, MagicMock]:
    ...
    idx = ElasticsearchDocumentIndex.__new__(ElasticsearchDocumentIndex)
    client = MagicMock()
    idx._client = client
...
def test_pair_converts_secondary_missing_to_typed_signal() -> None:
    secondary = MagicMock()
    secondary.update.side_effect = ElasticsearchDocumentMissingError(
        ["chunk-1"], ["doc-1"]
    )
...
def test_pair_propagates_other_secondary_errors() -> None:
    secondary = MagicMock()
    secondary.update.side_effect = ElasticsearchUpdateError("boom")
...
def test_pair_surfaces_primary_missing_during_instant_backfill() -> None:
    ...
    primary.update.side_effect = ElasticsearchDocumentMissingError(
        ["chunk-1"], ["doc-1"]
    )
