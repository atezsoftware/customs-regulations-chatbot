...
class TestGetElasticsearchDocChunkIdSingleTenant:
    def test_basic(self) -> None:
        result = get_elasticsearch_doc_chunk_id(
    ...
    def test_short_doc_id_not_hashed(self) -> None:
        ...
        doc_id = "short-id"
        result = get_elasticsearch_doc_chunk_id(
            SINGLE_TENANT_STATE, doc_id, chunk_index=0
        )
    ...
    def test_long_doc_id_hash_is_deterministic(self) -> None:
        ...
        result1 = get_elasticsearch_doc_chunk_id(
            SINGLE_TENANT_STATE, doc_id, chunk_index=5
        )
        result2 = get_elasticsearch_doc_chunk_id(
            SINGLE_TENANT_STATE, doc_id, chunk_index=5
        )
    ...
    def test_long_doc_id_different_inputs_produce_different_hashes(self) -> None:
        ...
        result_a = get_elasticsearch_doc_chunk_id(
            SINGLE_TENANT_STATE, doc_id_a, chunk_index=0
        )
        result_b = get_elasticsearch_doc_chunk_id(
...
class TestGetElasticsearchDocChunkIdMultiTenant:
    def test_includes_tenant_prefix(self) -> None:
        result = get_elasticsearch_doc_chunk_id(
            MULTI_TENANT_STATE, "mydoc", chunk_index=0
        )
    ...
    def test_long_doc_id_is_hashed_multitenant(self) -> None:
        ...
        result = get_elasticsearch_doc_chunk_id(
            MULTI_TENANT_STATE, doc_id, chunk_index=0
        )
    ...
    def test_different_tenants_produce_different_ids(self) -> None:
        ...
        result_a = get_elasticsearch_doc_chunk_id(tenant_a, "same-doc", chunk_index=0)
        result_b = get_elasticsearch_doc_chunk_id(tenant_b, "same-doc", chunk_index=0)
        assert result_a != result_b


class TestGetElasticsearchDocChunkIdEdgeCases:
    def test_chunk_index_zero(self) -> None:
        result = get_elasticsearch_doc_chunk_id(
            SINGLE_TENANT_STATE, "doc", chunk_index=0
        )
    ...
    def test_doc_id_with_only_special_chars_raises(self) -> None:
        ...
        with pytest.raises(ValueError, match="empty after filtering"):
            get_elasticsearch_doc_chunk_id(
                SINGLE_TENANT_STATE, "###???///", chunk_index=0
            )
