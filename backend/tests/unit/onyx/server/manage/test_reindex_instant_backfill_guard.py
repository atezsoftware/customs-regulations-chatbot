...
@pytest.mark.parametrize(
    ("switchover_type", "expects_canonical_projection"),
    [
        (SwitchoverType.REINDEX, True),
        (SwitchoverType.ACTIVE_ONLY, True),
        (SwitchoverType.INSTANT, False),
    ],
)
def test_waiting_reindex_queues_canonical_projection_independent_of_contextual_rag(
    switchover_type: SwitchoverType,
    expects_canonical_projection: bool,
) -> None:
    current = _present(use_port_flow=True, backfill_source_id=None)
    future = MagicMock()
    future.id = 19
    future.use_port_flow = True
    future.switchover_type = switchover_type
    db_session = MagicMock()

    with (
        patch(f"{_MODULE}.validate_contextual_rag_model"),
        patch(f"{_MODULE}.get_current_search_settings", return_value=current),
        patch(f"{_MODULE}.get_secondary_search_settings", return_value=None),
        patch(f"{_MODULE}.create_search_settings", return_value=future),
        patch(f"{_MODULE}.get_all_document_indices", return_value=[]),
        patch(
            f"{_MODULE}.fetch_indexable_standard_connector_credential_pair_ids",
            return_value=[],
        ),
        patch(f"{_MODULE}.get_connector_credential_pairs", return_value=[]),
        patch(
            f"{_MODULE}.mark_regulatory_user_files_reconcile_pending__no_commit",
            return_value=65,
        ) as mark_regulatory,
    ):
        result = set_new_search_settings(
            _request(switchover_type), _=MagicMock(), db_session=db_session
        )

    assert result.id == future.id
    if expects_canonical_projection:
        mark_regulatory.assert_called_once_with(db_session)
    else:
        mark_regulatory.assert_not_called()
