"""External dependency unit tests for ElasticsearchIndexClient.

These tests assume Elasticsearch is running and test all implemented methods
using real schemas and search queries from the codebase.
"""
...
import onyx.document_index.elasticsearch.client as client_module
...
from onyx.document_index.elasticsearch.client import (
    ...
    ElasticsearchServerSideTimeout,
...
from onyx.document_index.elasticsearch.index_reclaim import (
    ReclaimOutcome,
    reclaim_index_data,
)
...
def _patch_global_tenant_state(monkeypatch: pytest.MonkeyPatch, state: bool) -> None:
    ...
    monkeypatch.setattr("onyx.document_index.elasticsearch.schema.MULTI_TENANT", state)
...
def _patch_hybrid_search_subquery_configuration(
    ...
    """
    monkeypatch.setattr(
        "onyx.document_index.elasticsearch.constants.HYBRID_SEARCH_SUBQUERY_CONFIGURATION",
        configuration,
    )
    monkeypatch.setattr(
        "onyx.document_index.elasticsearch.search.HYBRID_SEARCH_SUBQUERY_CONFIGURATION",
        configuration,
    )


def _patch_hybrid_search_normalization_method(
    monkeypatch: pytest.MonkeyPatch, method: HybridSearchNormalizationMethod
) -> None:
    """
    Patches HYBRID_SEARCH_NORMALIZATION_METHOD wherever necessary for this
    test file.
    """
    monkeypatch.setattr(
        "onyx.document_index.elasticsearch.constants.HYBRID_SEARCH_NORMALIZATION_METHOD",
        method,
    )
    monkeypatch.setattr(
        "onyx.document_index.elasticsearch.search.HYBRID_SEARCH_NORMALIZATION_METHOD",
        method,
    )


def _patch_elasticsearch_match_highlights_disabled(
    monkeypatch: pytest.MonkeyPatch, disabled: bool
) -> None:
    """
    Patches ELASTICSEARCH_MATCH_HIGHLIGHTS_DISABLED wherever necessary for this
    test file.
    """
    monkeypatch.setattr(
        "onyx.configs.app_configs.ELASTICSEARCH_MATCH_HIGHLIGHTS_DISABLED",
        disabled,
    )
    monkeypatch.setattr(
        "onyx.document_index.elasticsearch.search.ELASTICSEARCH_MATCH_HIGHLIGHTS_DISABLED",
        disabled,
    )
...
def elasticsearch_available() -> None:
    """Verifies Elasticsearch is running, skips all tests if not."""
...
def test_client(
    elasticsearch_available: None,  # noqa: ARG001
) -> Generator[ElasticsearchIndexClient, None, None]:
    """Creates an Elasticsearch client for testing with automatic cleanup."""
    test_index_name = f"test_index_{uuid.uuid4().hex[:8]}"
    client = ElasticsearchIndexClient(index_name=test_index_name)
...
@pytest.fixture(scope="function")
def hybrid_normalization() -> Generator[None, None, None]:
    """Marks tests that exercise both native and client-side hybrid fusion."""
    yield


class TestElasticsearchClient:
    """Tests for ElasticsearchIndexClient."""

    def test_create_index(self, test_client: ElasticsearchIndexClient) -> None:
    ...
    def test_delete_existing_index(self, test_client: ElasticsearchIndexClient) -> None:
    ...
    def test_delete_nonexistent_index(
        self, test_client: ElasticsearchIndexClient
    ) -> None:
    ...
    def test_index_exists(self, test_client: ElasticsearchIndexClient) -> None:
    ...
    def test_validate_index(self, test_client: ElasticsearchIndexClient) -> None:
    ...
    def test_put_mapping_idempotent(
        self, test_client: ElasticsearchIndexClient
    ) -> None:
    ...
    def test_update_settings(self, test_client: ElasticsearchIndexClient) -> None:
    ...
    def test_get_settings(self, test_client: ElasticsearchIndexClient) -> None:
    ...
    def test_close_and_open_index(self, test_client: ElasticsearchIndexClient) -> None:
    ...
    def test_close_nonexistent_index(
        self, test_client: ElasticsearchIndexClient
    ) -> None:
    ...
    def test_open_nonexistent_index(
        self, test_client: ElasticsearchIndexClient
    ) -> None:
    ...
    def test_index_document(
        self, test_client: ElasticsearchIndexClient, monkeypatch: pytest.MonkeyPatch
    ) -> None:
    ...
    def test_bulk_index_duplicate_documents(
        ...
        and elasticsearch-py's ``bulk`` is called with raise_on_error=True, so any
        ...
        # raise.
        with pytest.raises(BulkIndexError, match="failed to index"):
            test_client.bulk_index_documents(documents=docs, tenant_state=tenant_state)
    ...
    def test_bulk_index_documents_raises_on_success_count_mismatch(
        ...
        """
        Tests that bulk_index_documents raises ElasticsearchIndexError when
        Elasticsearch reports no errors but the number of successful operations
        ...
        # Under test and postcondition.
        with pytest.raises(ElasticsearchIndexError, match="does not match"):
            test_client.bulk_index_documents(documents=docs, tenant_state=tenant_state)
    ...
    def test_bulk_update_surface_document_missing(
        ...
        ElasticsearchDocumentMissingError when the caller opts in (reindex port)."""
        _patch_global_tenant_state(monkeypatch, False)
        tenant_state = TenantState(tenant_id=POSTGRES_DEFAULT_SCHEMA, multitenant=False)
        mappings = DocumentSchema.get_document_schema(
            vector_dimension=128, multitenant=tenant_state.multitenant
        ...
        missing_id = get_elasticsearch_doc_chunk_id(
            tenant_state=tenant_state,
        ...
        with pytest.raises(ElasticsearchUpdateError):
            test_client.bulk_update_documents(
                document_chunk_ids=[missing_id],
                properties_to_update={"hidden": True},
            )

        # Opted-in: surfaced as ElasticsearchDocumentMissingError instead.
        with pytest.raises(ElasticsearchDocumentMissingError) as exc:
            test_client.bulk_update_documents(
                document_chunk_ids=[missing_id],
                properties_to_update={"hidden": True},
    ...
    def test_get_document(
        ...
        # Under test.
        doc_chunk_id = get_elasticsearch_doc_chunk_id(
            tenant_state=tenant_state,
    ...
    def test_bulk_update_nonexistent_documents(
        ...
        Elasticsearch returns a 404 ``document_missing_exception`` per missing
        chunk. Because the status is < 500, the implementation classifies these
        as fatal (non-retryable) and raises ElasticsearchUpdateError.
        """
        # Precondition.
        _patch_global_tenant_state(monkeypatch, False)
        tenant_state = TenantState(tenant_id=POSTGRES_DEFAULT_SCHEMA, multitenant=False)
        ...
        with pytest.raises(ElasticsearchUpdateError, match="fatal error"):
            test_client.bulk_update_documents(
                document_chunk_ids=[
    ...
    def test_bulk_update_nonexistent_documents_ignore_missing(
        ...
        test_client.index_document(document=doc, tenant_state=tenant_state)
        existing_id = get_elasticsearch_doc_chunk_id(
            tenant_state=tenant_state,
            document_id=doc.document_id,
            chunk_index=doc.chunk_index,
            max_chunk_size=doc.max_chunk_size,
    ...
    def test_bulk_update_documents_retries_retryable_errors(
        ...
        Guards the workaround for the Elasticsearch 3.4.0 knn/derived_source bug.
        The transient errors are very hard to trigger from a test against a
        healthy Elasticsearch, so we monkeypatch ``bulk`` to inject one.
        """
        # Precondition.
        _patch_global_tenant_state(monkeypatch, False)
        tenant_state = TenantState(tenant_id=POSTGRES_DEFAULT_SCHEMA, multitenant=False)
        ...
        def fake_bulk(
            ...
            # Subsequent call: actually run the update against Elasticsearch.
        ...
        # the retry runs against the real Elasticsearch and succeeds.
        test_client.bulk_update_documents(
    ...
    def test_bulk_update_documents_raises_on_success_count_mismatch(
        ...
        """
        Tests that bulk_update_documents raises ElasticsearchUpdateError when
        Elasticsearch reports no errors but the number of successful operations
        ...
        # Under test and postcondition.
        with pytest.raises(ElasticsearchUpdateError, match="does not match"):
            test_client.bulk_update_documents(
                document_chunk_ids=[
    ...
    def test_hybrid_search_configurations_and_normalizers(
        self,
        test_client: ElasticsearchIndexClient,
        hybrid_normalization: None,  # noqa: ARG002
        monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        """Tests all hybrid search configurations and normalizers."""
        # Precondition.
        _patch_global_tenant_state(monkeypatch, False)
        _patch_elasticsearch_match_highlights_disabled(monkeypatch, False)
        tenant_state = TenantState(tenant_id=POSTGRES_DEFAULT_SCHEMA, multitenant=False)
        mappings = DocumentSchema.get_document_schema(
            vector_dimension=128, multitenant=tenant_state.multitenant
        )
        ...
        for configuration in HybridSearchSubqueryConfiguration:
            ...
            for method in HybridSearchNormalizationMethod:
                _patch_hybrid_search_normalization_method(monkeypatch, method)
                method_name, _ = get_normalization_method_and_config()
                ...
                results = test_client.search(
                    body=search_body, normalization_method=method_name
                )
    ...
    def test_search_empty_index(
        ...
        # Under test.
        results = test_client.search(body=search_body, normalization_method=method_name)

        # Postcondition.
        assert len(results) == 0

    def test_hybrid_search_with_normalization_and_filters(
        self,
        test_client: ElasticsearchIndexClient,
        hybrid_normalization: None,  # noqa: ARG002
        monkeypatch: pytest.MonkeyPatch,
    ...
    def test_hybrid_search_with_normalization_and_filters_returns_chunks_with_related_content_first(
        self,
        test_client: ElasticsearchIndexClient,
        hybrid_normalization: None,  # noqa: ARG002
        monkeypatch: pytest.MonkeyPatch,
        ...
        """
        Tests search with a normalization method and filters returns chunks
    ...
    def test_random_search(
        ...
        # Under test.
        results = test_client.search(body=query_body, normalization_method=None)
    ...
    def test_keyword_search(
        ...
        # Under test.
        results = test_client.search(body=search_body, normalization_method=None)
...
class TestSearchFailureMetrics:
    """Regression coverage for the Elasticsearch search failure-rate metric.

    Before the fix, ``self._log_search_result_perf(..., raise_on_timeout=True)``
    ran after the metrics ``try/except`` block. When Elasticsearch returned
    ``"timed_out": true`` in the response body, the helper raised
    ``RuntimeError`` outside the ``except`` arm, so the failure never reached
    ``record_elasticsearch_search_error`` and ``observe_elasticsearch_search`` had
    ...
    def _timed_out_response() -> dict[str, Any]:
        """
        A minimally-valid mock Elasticsearch search response with timed_out=true.
        """
        return {
            "took": 1234,
            "timed_out": True,
    ...
    def _make_client_with_canned_response(
        ...
        """
        Patches the Elasticsearch class imported by ``client_module`` so that
        ``ElasticsearchIndexClient.__init__`` constructs a mocked underlying client
        whose ``.search`` returns a mocked response. Lets us drive the timeout
        code path without hitting a real Elasticsearch.
        """
        mock_underlying = MagicMock()
        mock_underlying.search.return_value = response
        monkeypatch.setattr(
            client_module, "Elasticsearch", MagicMock(return_value=mock_underlying)
        )
        return ElasticsearchIndexClient(index_name="test_index")
    ...
    def test_search_records_error_on_server_side_timeout(
        ...
        monkeypatch.setattr(
            client_module, "record_elasticsearch_search_error", record_error_mock
        )
        monkeypatch.setattr(client_module, "observe_elasticsearch_search", observe_mock)
        ...
        # Under test.
        with pytest.raises(ElasticsearchServerSideTimeout, match="timed out"):
            client.search(
                body={},
        ...
        assert recorded_search_type == ElasticsearchSearchType.HYBRID
        assert isinstance(recorded_exc, ElasticsearchServerSideTimeout)
    ...
    def test_search_for_document_ids_records_error_on_server_side_timeout(
        ...
        assert record_error_mock.call_count == 1
        recorded_search_type, recorded_exc = record_error_mock.call_args.args
        assert recorded_search_type == ElasticsearchSearchType.KEYWORD
        assert isinstance(recorded_exc, ElasticsearchServerSideTimeout)
        assert observe_mock.call_count == 0
    ...
    def test_pit_scan_raises_on_server_timeout(
        ...
        with pytest.raises(ElasticsearchServerSideTimeout):
            test_client.fetch_chunks_for_doc_ids(pit_id, ["doc-a"], page_size=4)
...
class TestIndexReclaimPrimitive:
    ...
    def _create_mt_index(
        self, client: ElasticsearchIndexClient, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        _patch_global_tenant_state(monkeypatch, True)
        mappings = DocumentSchema.get_document_schema(
            vector_dimension=128, multitenant=True
    ...
    def test_reclaim_incomplete_when_docs_remain(
        ...
        monkeypatch.setattr(ElasticsearchIndexClient, "count_by_query", lambda *_: 3)
    ...
    def test_reclaim_missing_index_is_complete(
        self,
        elasticsearch_available: None,  # noqa: ARG002
    ) -> None:
    ...
    def test_reclaim_multi_tenant_drains_over_multiple_calls(
        ...
        self._create_mt_index(test_client, monkeypatch)
        monkeypatch.setattr(
            "onyx.document_index.elasticsearch.index_reclaim.OLD_INDEX_RECLAIM_DELETE_BATCH_SIZE",
            2,
        )
        ts = TenantState(tenant_id=f"tenant_{uuid.uuid4().hex}", multitenant=True)
        self._index_docs(test_client, ts, "doc", 5)
