...
def test_waiting_reindex_ports_only_legacy_files_for_mixed_user(
    db_session: Session, future_ss_id: int, port_user: User
) -> None:
    regulatory = _make_user_file(db_session, port_user.id)
    _make_regulatory_chunk(db_session, regulatory)
    legacy = _make_user_file(db_session, port_user.id)
    attempt_id = create_port_attempt(
        db_session, None, future_ss_id, port_user_id=port_user.id
    ).id
    ...
    mock_copier.copy_doc_batch.assert_called_once()
    assert mock_copier.copy_doc_batch.call_args.args[0] == [str(legacy.id)]
    db_session.expire_all()
    row = db_session.get(PortAttempt, attempt_id)
    assert row is not None
    assert row.status == PortAttemptStatus.SUCCESS
    assert row.docs_ported == 1
    assert row.last_processed_doc_id == str(legacy.id)


def test_regulatory_only_user_completes_zero_doc_port_but_waits_for_projection(
    db_session: Session, future_ss_id: int, port_user: User
) -> None:
    ...
    regulatory = _make_user_file(db_session, port_user.id)
    _make_regulatory_chunk(db_session, regulatory)
    regulatory.secondary_reconcile_pending = True
    db_session.commit()
    attempt_id = create_port_attempt(
        db_session, None, future_ss_id, port_user_id=port_user.id
    ).id
    ...
    mock_copier.copy_doc_batch.assert_not_called()
    db_session.expire_all()
    row = db_session.get(PortAttempt, attempt_id)
    assert row is not None
    assert row.status == PortAttemptStatus.SUCCESS
    assert (row.docs_ported or 0) == 0
    assert row.last_processed_doc_id is None
    assert _port_swap_ready(db_session, future_ss, [], [port_user.id]) is False

    regulatory = db_session.get(UserFile, regulatory.id)
    assert regulatory is not None
    regulatory.secondary_reconcile_pending = False
    db_session.commit()
    assert _port_swap_ready(db_session, future_ss, [], [port_user.id]) is True


def test_instant_port_keeps_regulatory_files_in_generic_copy(
    db_session: Session, future_ss_id: int, port_user: User
) -> None:
    future_ss = db_session.get(SearchSettings, future_ss_id)
    assert future_ss is not None
    ...
    mock_copier.copy_doc_batch.assert_called_once()
    assert mock_copier.copy_doc_batch.call_args.args[0] == [str(regulatory.id)]
