...
def _make_regulatory_chunk(db_session: Session, user_file: UserFile) -> RegulatoryChunk:
...
def test_user_file_port_batch_excludes_only_exact_canonical_regulatory_id(
    db_session: Session, port_user: User
) -> None:
    regulatory = _make_user_file(db_session, port_user.id)
    _make_regulatory_chunk(db_session, regulatory)
    legacy_duplicate = _make_user_file(db_session, port_user.id)
    legacy_duplicate.file_id = regulatory.file_id
    legacy_duplicate.name = regulatory.name
    ordinary_legacy = _make_user_file(db_session, port_user.id)
    db_session.commit()

    all_ids = sorted(
        [str(regulatory.id), str(legacy_duplicate.id), str(ordinary_legacy.id)]
    )
    page = get_user_file_ids_for_user_batch(
        db_session,
        port_user.id,
        ...
        limit=10,
        up_to_id=all_ids[-1],
        exclude_regulatory_files=True,
    )

    assert set(page) == {str(legacy_duplicate.id), str(ordinary_legacy.id)}
    assert str(regulatory.id) not in page
    # The snapshot bound and user scope still include canonical-only files.
    assert get_max_user_file_id_for_user(db_session, port_user.id) == all_ids[-1]
    assert port_user.id in fetch_port_scope_user_ids(db_session)
...
def test_survival_filter_drops_file_that_became_canonical_regulatory(
    db_session: Session, port_user: User
) -> None:
    legacy = _make_user_file(db_session, port_user.id, UserFileStatus.COMPLETED)
    assert filter_existing_user_file_ids(
        db_session,
        port_user.id,
        [str(legacy.id)],
        exclude_regulatory_files=True,
    ...
    _make_regulatory_chunk(db_session, legacy)
