...
def test_augmentation_enrich_on_generates_and_reembeds(
    ...
    assert result.doc_summary == f"SUMMARY.{RETURN_SEPARATOR}"
    assert result.chunk_context == f"{RETURN_SEPARATOR}CONTEXT."
    assert result.content == (
        f"My Title{RETURN_SEPARATOR}SUMMARY.{RETURN_SEPARATOR}"
        f"{bare}{RETURN_SEPARATOR}CONTEXT.{keyword}"
    )

    # the embedded text carries the new summaries; vector differs from strip-only
    enrich_embed_input = (
        f"My Title{RETURN_SEPARATOR}SUMMARY.{RETURN_SEPARATOR}"
        f"{bare}{RETURN_SEPARATOR}CONTEXT.{semantic}"
    )
...
def test_augmentation_mixed_docs_enrich_per_document(
    ...
    assert results[0].doc_summary == (f"[doc-a:a-first a-second]{RETURN_SEPARATOR}")
    assert results[1].doc_summary == f"[doc-b:b-only]{RETURN_SEPARATOR}"
    assert results[2].doc_summary == (f"[doc-a:a-first a-second]{RETURN_SEPARATOR}")


def test_regulatory_context_uses_temporal_snapshot(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr("onyx.indexing.port_reembed.USE_CHUNK_SUMMARY", False)
    monkeypatch.setattr("onyx.indexing.port_reembed.USE_DOCUMENT_SUMMARY", True)

    def _fake_enrich(
        chunks: list[DocAwareChunk], **_kwargs: object
    ) -> list[DocAwareChunk]:
        for chunk in chunks:
            chunk.doc_summary = f"[{chunk.source_document.get_text_content()}] "
        return chunks
    ...
    old = _stored_chunk(
        ...
        chunk_index=0,
        title=None,
        regulatory_chunk_id="old",
        heading_path=["Madde 1"],
        validity_end_date=datetime.datetime(2023, 12, 31),
    )
    new = _stored_chunk(
        "new rule",
        chunk_index=1,
        title=None,
        regulatory_chunk_id="new",
        heading_path=["Madde 1"],
        validity_start_date=datetime.datetime(2024, 1, 1),
    )
    common = _stored_chunk(
        "unchanged rule",
        chunk_index=2,
        title=None,
        regulatory_chunk_id="common",
    ...
    ctx = AugmentationReembedContext(
        ...
        tokenizer=MagicMock(),
        chunk_token_limit=128,
        contextual_rag_reserved_tokens=50,
    )
    ...
    assert "old rule" in results[0].doc_summary
    assert "unchanged rule" in results[0].doc_summary
    assert "new rule" not in results[0].doc_summary
    assert "new rule" in results[1].doc_summary
    assert "unchanged rule" in results[1].doc_summary
    assert "old rule" not in results[1].doc_summary
    assert "new rule" in results[2].doc_summary
    assert "old rule" not in results[2].doc_summary
    assert all(result.document_id == "doc-1" for result in results)


def test_regulatory_context_uses_remaining_capacity_without_losing_legal_text(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr("onyx.indexing.port_reembed.USE_CHUNK_SUMMARY", False)
    monkeypatch.setattr("onyx.indexing.port_reembed.USE_DOCUMENT_SUMMARY", True)
    ...
    near_full = _stored_chunk(
        "x" * 424,
        chunk_index=0,
        title=None,
        regulatory_chunk_id="near-full",
    )
    full = _stored_chunk(
        "y" * 512,
        chunk_index=1,
        title=None,
        regulatory_chunk_id="full",
    )
    short = _stored_chunk(
        "short rule",
        chunk_index=2,
        title=None,
        regulatory_chunk_id="short",
    )
    ctx = AugmentationReembedContext(
        future_enable_contextual_rag=True,
        future_embedding_tokenizer=_TOKENIZER,
    ...
    results = re_embed_chunks(
        [near_full, full, short],
        ReembedStrategy.AUGMENTATION,
        cast(IndexingEmbedder, _ContentVecEmbedder()),
        augmentation_ctx=ctx,
    )

    assert results[0].doc_summary == f"SUMMARY{RETURN_SEPARATOR}"
    assert results[0].content.removeprefix(results[0].doc_summary) == "x" * 424
    assert results[1].doc_summary == ""
    assert results[1].content == "y" * 512
    assert results[2].doc_summary == f"SUMMARY{RETURN_SEPARATOR}"
