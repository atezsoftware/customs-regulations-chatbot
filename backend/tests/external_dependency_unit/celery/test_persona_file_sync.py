...
def _create_regulatory_chunk(
    db_session: Session,
    user_file: UserFile,
) -> RegulatoryChunk:
    """Insert the canonical PostgreSQL row used by secondary projection."""

    chunk = RegulatoryChunk(
        id=f"rc_test_{uuid4().hex}",
...
class TestDualWriteFallbackHelper:
    ...
    def test_supplies_content_returns_true(
        ...
        """With a target, canonical chunks are projected into FUTURE and the
        reconcile flag can clear without loading the uploaded blob."""
        user = create_test_user(db_session, "fb_ok")
        uf = _create_completed_user_file(db_session, user)
        _create_regulatory_chunk(db_session, uf)
        try:
            target = MagicMock()
            index_secondary = MagicMock()
            load_docs = MagicMock()
            with (
                ...
                patch(_PATCH_LOAD_DOCS, load_docs),
                patch(_PATCH_INDEX_SECONDARY, index_secondary),
            ):
                result = _supply_user_file_to_secondary(
                    str(uf.id), POSTGRES_DEFAULT_SCHEMA_STANDARD_VALUE
            ...
            assert index_secondary.call_args.args[1] is target
