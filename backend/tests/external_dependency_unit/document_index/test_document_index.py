...
class TestDocumentIndexNew:
    ...
    def test_relationship_query_discovers_unknown_regulatory_provision(
        self,
        document_indices: list[DocumentIndexNew],
        tenant_context: None,  # noqa: ARG002
    ) -> None:
        """Contextual retrieval finds operative text without its provision id."""

        query = (
            "izin veren idare uygunsuzluk sürerken statünün kullanılmasını "
            "geçici olarak durdurabilir mi"
        )
        operative_text = (
            "Esaslı gözetim ihlali saptandığında, yetkilendirme tasarrufu kararı "
            "tesis eden merci tarafından giderim tamamlanıncaya değin askıya alınabilir."
        )
        contextual_bridge = (
            "Bu hüküm, izin veren idarenin devam eden uygunsuzluk sırasında bir "
            "statünün kullanılmasını geçici olarak durdurma yetkisini açıklar."
        )
        assert "madde" not in query.casefold()
        assert "askıya" not in query.casefold()
        assert "yetkilendirme tasarrufu" not in query.casefold()

        partial_contexts = [
            "İzin başvurularının kayıt usulünü açıklar.",
            "İdare tarafından tutulacak sicilin biçimini düzenler.",
            "Statünün yıllık bildirim dönemini belirler.",
            "Geçici görevlilerin çalışma çizelgesini düzenler.",
            "Uygunsuzluk raporunun arşivlenme biçimini açıklar.",
        ]

        for document_index in document_indices:
            doc_id = f"unknown_provision_{uuid.uuid4().hex[:8]}"
            target_chunk_id = 7
            chunks = [
                make_chunk(
                    doc_id,
                    chunk_id=index,
                    content=f"Bağımsız kayıt ve bildirim hükmü {index}.",
                    chunk_context=f"\n\n{partial_contexts[index % len(partial_contexts)]}",
                    regulatory_chunk_id=f"{doc_id}-regulatory-{index}",
                    heading_path=["SENTETİK UYUM DÜZENLEMESİ", f"KURAL {index}"],
                )
                for index in range(16)
            ]
            chunks[target_chunk_id] = make_chunk(
                doc_id,
                chunk_id=target_chunk_id,
                content=operative_text,
                chunk_context=f"\n\n{contextual_bridge}",
                regulatory_chunk_id=f"{doc_id}-regulatory-target",
                heading_path=["SENTETİK UYUM DÜZENLEMESİ", "UYUM TEDBİRLERİ"],
            )
            ...
            while time.monotonic() < deadline:
                retrieved = document_index.keyword_retrieval(
                    query=query,
                    filters=filters,
                    num_to_retrieve=12,
                )
                if any(chunk.chunk_id == target_chunk_id for chunk in retrieved):
                    break
                time.sleep(0.25)

            assert len(retrieved) == 12
            target = next(
                chunk for chunk in retrieved if chunk.chunk_id == target_chunk_id
            )
