...
# Vendor-owned upstream services (telemetry, release notes, cloud billing and
# hosted error reporting) are disabled unless a deployment explicitly opts in.
# This does not affect configured LLM, embedding, connector, or SMTP providers.
DISABLE_ONYX_UPSTREAM_CONNECTIONS = (
    os.environ.get("DISABLE_ONYX_UPSTREAM_CONNECTIONS", "true").lower() != "false"
)

# Set up Sentry integration (for error logging). Keep it off together with the
# other vendor upstreams even if a stale DSN remains in the environment.
SENTRY_DSN = (
    None
    if DISABLE_ONYX_UPSTREAM_CONNECTIONS
    else os.environ.get("SENTRY_DSN")
)
