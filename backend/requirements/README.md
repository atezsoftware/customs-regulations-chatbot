...
2. Add/update/remove dependencies in the appropriate section:
   ...
   - `[dependency-groups.backend-runtime]` for API/chat/search runtime dependencies
   - `[dependency-groups.document-import]` for the heavy document parser stack
   - `[dependency-groups.browser-automation]` for Playwright-based connectors
   - `[dependency-groups.backend]` for the backwards-compatible full backend
...
uv export --no-emit-project --no-default-groups --group backend-runtime -o backend/requirements/runtime.txt
