      ...
      <main className="flex-1 overflow-y-auto">
        {selectedId === null ? (
          <div className="flex h-full flex-col items-center justify-center px-6 text-center">
            <p className="text-lg font-medium text-slate-700">Pick a dataset</p>
            <p className="mt-1 max-w-sm text-sm text-slate-400">
              Or create one to start uploading files and generating an index.
            </p>
          </div>
                ...
                <div className="flex shrink-0 gap-2">
                  <Button
                    ...
                    disabled={
                      ...
                      !detail.files.some(file => file.storageStatus === 'stored')
                    }
                    onClick={() => void handleStartChunking()}
                  >
                    {indexStatus?.status === 'chunking'
                      ? 'Generating chunks...'
                      : indexStatus?.status === 'completed' || indexStatus?.status === 'stale'
                        ? 'Chunk new files'
                        : 'Generate chunks'}
                  </Button>
                  <Button
