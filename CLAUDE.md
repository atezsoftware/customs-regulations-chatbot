...
- `OPENROUTER_MAX_OUTPUT_TOKENS` (optional, `core-api`, default `8000`) — output ceiling sent as the endpoint-compatible parameter: `max_completion_tokens` for modern OpenAI reasoning models and `max_tokens` for other model families. If OpenRouter reports endpoint ineligibility or a typed/generic provider-side parameter rejection, the client keeps the exact model, messages, schema, and privacy policy while trying the alternate token spelling, then a form without optional reasoning/temperature, and finally no explicit token ceiling. Content-policy, refusal, context-length, and token-limit failures are never treated as parameter compatibility. Pydantic's validation-neutral annotations beside `$ref` are removed before structured schemas are sent because OpenAI-compatible endpoints reject `$ref` siblings; validation keywords are never discarded. `X-OpenRouter-Metadata: enabled` exposes routing/error diagnostics without enabling prompt logging. This matters with `provider.require_parameters=true`: under an OpenAI ZDR policy the eligible Azure endpoint supports `max_completion_tokens`, so sending only `max_tokens` would incorrectly remove the policy-compliant endpoint.
...
- `FS_EXPLORER_LLM_RETRY_ATTEMPTS` / `FS_EXPLORER_LLM_RETRY_BACKOFF_SECONDS` (optional, `core-api`, default 3 / 2s) — in `llm/gemini.py`, both `generate_structured()` (tool-planning/compaction calls) and `stream_text()` (the final-answer stream) retry on provider/network `429 RESOURCE_EXHAUSTED`, `503 UNAVAILABLE`, and timeout failures, waiting `FS_EXPLORER_LLM_RETRY_BACKOFF_SECONDS` between attempts. The application adds no wall-clock deadline. `stream_text()` only retries if no text has been yielded to the caller yet — once real answer text has streamed out, a later failure is not retried (would risk duplicated/out-of-order output) and instead propagates so the caller's fallback path handles it
