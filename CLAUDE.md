...
- `OPENROUTER_MAX_OUTPUT_TOKENS` (optional, `core-api`, default `8000`) — output ceiling sent as the endpoint-compatible parameter: `max_completion_tokens` for modern OpenAI reasoning models and `max_tokens` for the other configured model families. This distinction matters when `provider.require_parameters=true`: under an OpenAI ZDR/privacy policy the eligible Azure endpoint supports `max_completion_tokens`, so sending `max_tokens` would incorrectly remove the last policy-compliant endpoint and produce a “No endpoints available matching your guardrail restrictions and data policy” error. Reasoning-heavy models also spend completion tokens on hidden chain-of-thought before emitting visible content; without an explicit ceiling they can exhaust the upstream default entirely on thinking and return empty content.
