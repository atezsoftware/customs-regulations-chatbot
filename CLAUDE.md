...
- **workflow.py**: Event-driven orchestration using `llama-index-workflows`. Defines `FsExplorerWorkflow` with steps: `start_exploration`, `go_deeper_action`, `tool_call_action`, `receive_human_answer`. Uses singleton agent via `get_agent()`. `resume_agent_run()` continues an already-in-progress agent directly (no `InputEvent`/`start_exploration` re-entry) — used only to resume a run interrupted by a dropped connection, manual stop, or an unrecoverable error; see `runs.py` and `server.py`'s `/ws/explore` resume path. When a question has both `use_index` and `enable_semantic` set, `start_exploration()` unconditionally hands it to `agent.py`'s context-isolated multi-agent research orchestrator (`prepare_multi_agent_indexed()`, backed by `multi_agent.py`'s `MultiAgentResearchOrchestrator` — a model-controlled plan-then-task-DAG flow where every LLM role gets a fresh one-turn history and returns a typed artifact, never a shared chat transcript) — this is the single indexed research flow, not one of several; a failure inside it propagates as a real error instead of switching pipelines. Invalid plans are returned to the global planner with exact validation errors until it repairs the full graph; there is no alternate planner path. The global planner enumerates every material answer requirement; task coordinators decide how each need should be researched; each persistent search agent owns its need and receives every `indexed_search` tool result, including successful ones, before it chooses exactly one next search or explicitly stops because the need is supported/corpus-exhausted. Empty results and tool failures are returned to the same agent, which can change terminology, jurisdiction, instrument, scope, date, exception, cross-reference strategy, or retry the failed tool call. Malformed model-supplied metadata filters are discarded and corpus-specific invalid filters retry the same semantic query without filtering, so an optional filter cannot create a zero-evidence retry loop. The scheduler never invents a query. Production has no task, worker, wave, search-count, LLM-call, claim, artifact-count, or wall-clock stopping budget; Core WebSocket and browser SSE heartbeats keep slow stages connected. Parent agents receive complete verbatim evidence reports without presentation citations, and final synthesis answers from those reports without adding citations. Material facts missing from the user's scenario are announced and preserved as conditional unknowns rather than guessed. Metadata-only and non-indexed questions still use the classic iterative loop below (`go_deeper_action`/`tool_call_action`, driven by `agent.py`'s `take_action()`).

- **runs.py**: In-memory registry (`register_run`/`get_run`/`remove_run`, TTL-swept) of interrupted-but-resumable `/ws/explore` runs, keyed by `run_id`. A run is only registered when it made real progress (`agent.step_count > 0`) and didn't reach a real terminal state; a clean `complete` event always removes it. Process-local, like `new_workflow()`'s per-request `ResourceManager` — a resume only works if it lands back on the same `core-api` pod that held the interrupted run.

- **agent.py**: `FsExplorerAgent` talks to the LLM via the provider-agnostic `LLMClient` interface (`llm/base.py`; `llm/gemini.py` is the only implementation today). Chat history accumulates in `_chat_history` as a list of `ChatTurn` (provider-agnostic, not a Gemini-specific type). `take_action()` requests a structured JSON `Action`; `TokenUsage` tracks input/output/**thinking** tokens and costs. Before handing back a proposed `StopAction`, `take_action()` runs it past `_verify_answer_completeness()` — one extra `CompletenessCheck`-schema call (prompted with `COMPLETENESS_CHECK_PROMPT`) asking whether the gathered evidence actually supports a complete answer; a rejected stop gets a gap note appended to history and the loop continues for another real action instead of finalizing early, bounded by `FS_EXPLORER_MAX_STOP_REJECTIONS` (see Environment below). Also contains the `TOOLS` registry, `SYSTEM_PROMPT`, and indexed tool functions (`semantic_search`, `get_document`, `list_indexed_documents`). Index context is managed via `set_index_context()`/`clear_index_context()`, backed by `contextvars.ContextVar`s (not plain module globals — those raced across concurrently-running chats in the same process; see `_INDEX_CONTEXT_VAR`'s docstring in `agent.py`). Tool calls that miss the index (`preview_file`/`parse_file`/`scan_folder`) return a short "not indexed yet" message rather than falling back to raw Docling parsing — that capability lives only in `core-indexer` now. `set_effort()`/`get_effort()` (another `contextvars.ContextVar`, same per-request pattern) hold the user-selected `EffortLevel` (`"low"|"medium"|"high"`, default `"medium"`) that `semantic_search` uses to clamp its `limit` (a hard ceiling now, not just a model-suppliable hint) and the retrieval over-fetch multiplier passed to `IndexedQueryEngine.search()` — see `search/query.py` and `rerank.py` below.
- **rerank.py**: `ChunkReranker`/`get_reranker()` — a cross-encoder reranking step (OpenRouter's hosted `cohere/rerank-4-pro` by default) that sits between retrieval and the LLM, narrowing an over-fetched candidate pool down to the final `limit` with real relevance scores instead of `search/ranker.py`'s/`amendments/ranker.py`'s fixed linear semantic/metadata-score heuristic. `get_reranker()` is a lazily-built process-wide singleton that returns `None` (reranking disabled, callers fall back to the heuristic) when `FS_EXPLORER_RERANK_ENABLED=false` or `OPENROUTER_API_KEY` is unset — every call site treats a failed/absent rerank exactly like the heuristic path, so this never introduces a new failure mode, only a quality/cost upgrade when configured. Wired into `search/query.py`'s `IndexedQueryEngine._rank` (used by `semantic_search`) and `amendments/candidate_finder.py`'s `_rank` (used before `matcher.py`'s `confirm_match` sends candidates' full chunk text to the LLM — the highest-value rerank target, since those candidates aren't excerpt-truncated like `semantic_search`'s hits are).
...
- **server.py**: FastAPI server with WebSocket endpoint `/ws/explore` for real-time streaming, plus `/api/search`, `/api/document`, `/api/folders` and the bundled demo UI (`ui.html`). Slow workflow and final-answer awaits emit transport-only heartbeat events every 10 seconds; backend ignores these as research output and separately writes SSE keepalive comments to the browser, so only the final answer streams model text. Also `/api/index/document-chunks` — a pure-Postgres read (no Docling) duplicated from `core-indexer` so file-chunk lookups work wherever `core-api` is deployed, since `core-indexer` isn't wired into any deployed environment yet; `backend` calls this one on `core-api`, not `core-indexer`. All *other* `/api/index*` endpoints (build/refresh, embed, auto-profile) stay `core-indexer`-only. `/ws/explore`'s `start` event always carries a `run_id`; if the connection drops or the run hits an unrecoverable error before a `complete` event, `_register_if_resumable()` keeps that run's agent alive in `runs.py`'s registry, and a later connection can send `{"type": "resume", "run_id": ...}` instead of a normal `{"task": ...}` payload to pick it back up via `_run_resume_session()`/`workflow.resume_agent_run()` — `backend`'s `core-bridge.service.ts` exposes this as `GET .../messages/{messageId}/stream?resumeRunId=...`, surfaced in the UI as the "Continue" button on an errored/stopped message. The incoming `start`/`resume` message's `effort` field (`"low"|"medium"|"high"`, defaults to `"medium"` if missing/invalid) is read alongside `enable_semantic`/`enable_metadata` and applied via `agent.py`'s `set_effort()`; it's persisted on `runs.py`'s `RunRecord` too so a resumed run keeps the same effort level rather than reverting to the default — `backend`'s `chat-messages.controller.ts` exposes it as `GET .../messages/{messageId}/stream?effort=...` (frontend's `EffortSelector`, next to the model selector), only on a fresh run — a resume ignores any `effort` param since core-api already has the original run's.
...
- `OPENROUTER_API_KEY` (optional, `core-api`) — required for `llm/openrouter.py` (the benchmark judge, and the `"openrouter"` `FS_EXPLORER_LLM_PROVIDER` option) *and* for `rerank.py`'s cross-encoder reranking (see below) to actually activate; unset = the benchmark judge/OpenRouter provider error clearly when selected, while reranking silently falls back to the heuristic ranker instead of failing
- `OPENROUTER_BASE_URL` (optional, `core-api`, default `https://openrouter.ai/api/v1`) — shared by `llm/openrouter.py` and `rerank.py`
- `OPENROUTER_MAX_OUTPUT_TOKENS` (optional, `core-api`, default unset) — the application sends no output-token ceiling by default, so the selected model/provider can use its native capacity. If an operator explicitly sets a positive value, it is sent using the endpoint-compatible parameter (`max_completion_tokens` for modern OpenAI reasoning models, `max_tokens` for other families), and a real truncation signal doubles that requested budget on each recovery attempt. Parameter compatibility never changes the selected model, messages, schema, or privacy policy. Pydantic's validation-neutral annotations beside `$ref` are removed before structured schemas are sent; validation keywords are never discarded.
- `OPENROUTER_STRUCTURED_RETRIES` (optional, `core-api`, default unset/unlimited) — malformed, incomplete, empty, or schema-invalid structured responses are regenerated from the original request with the same model/schema/provider policy until a complete valid artifact arrives. Broken output is never copied into the correction prompt or exposed through Pydantic's raw `input_value`. A real `length`/`MAX_TOKENS` signal takes precedence even when the JSON prefix happens to validate. Refusal, safety stops, authentication errors, and other permanent rejections are not retried. Usage and provider cost are aggregated across every paid attempt.
- `FS_EXPLORER_LLM_MAX_CONCURRENCY` (optional, `core-api`, default unset/unlimited) — production adds no process-wide Gemini concurrency ceiling. A constrained deployment may opt into a positive shared semaphore value; unset uses a no-op async limiter.
- `FS_EXPLORER_LLM_RETRY_ATTEMPTS` / `FS_EXPLORER_LLM_RETRY_BACKOFF_SECONDS` (optional, `core-api`, default unlimited / 2s) — in `llm/gemini.py`, both `generate_structured()` and `stream_text()` retry provider/network `429 RESOURCE_EXHAUSTED`, `503 UNAVAILABLE`, and timeout failures without an application attempt or wall-clock ceiling. `stream_text()` only restarts when no text has yet been yielded, avoiding duplicated/out-of-order output; a later failure is propagated and the indexed workflow never substitutes a direct/deterministic answer.
- `GEMINI_STRUCTURED_RETRIES` (optional, `core-api`, default unset/unlimited) — incomplete, token-truncated, malformed, or schema-invalid Gemini structured output is regenerated from the original request until valid. The malformed output is never inserted into the correction prompt. Refusal and safety stops remain terminal.
...
- `FS_EXPLORER_CONTEXT_SUMMARY_MAX_TOKENS` (optional, `core-api`) — absolute token size (default 4000) at which `_maybe_summarize_history` compacts, checked in addition to the ratio above (whichever fires first). Since every `take_action()` call resends the *entire* history, this — not the step count — is what actually bounds cost on a long run: it re-triggers every time the resent history grows back past this size, so a run can legitimately take many steps without its per-call input size growing unbounded
- `FS_EXPLORER_MAX_STEPS` (optional, `core-api`) — safety-net ceiling (default 60), *not* a normal operating limit, on total agent decision-points (`take_action()` calls) in one run; only meant to catch a run that's escaped the near-duplicate-call guard (or hit some other bug) and would otherwise loop indefinitely — should essentially never fire for legitimate multi-step research, which is bounded on cost instead by `FS_EXPLORER_CONTEXT_SUMMARY_MAX_TOKENS` above. If it does fire, `agent.py`'s `forced_stop` flag is set and surfaced to the client as `complete`'s `incomplete: true` (even though there's no `error`), so the UI can still offer "Continue" — `FsExplorerAgent.grant_more_steps()` (called on resume) raises the ceiling again rather than the resumed run immediately re-hitting the same one
- `FS_EXPLORER_MAX_STOP_REJECTIONS` (optional, `core-api`, default 2) — cap in `agent.py`'s `take_action()` on how many times a proposed `StopAction` can be rejected by `_verify_answer_completeness()` (a cheap extra LLM call, prompted with `COMPLETENESS_CHECK_PROMPT`) before the run just accepts whatever the model proposes. Before returning a `StopAction`, `take_action()` now asks this separate check whether the gathered evidence actually supports a complete answer; if not, the stop is rejected, a note describing the gap is appended to history, and the loop continues for another real research action instead of letting an under-researched answer reach final synthesis — bounded by this cap (and, beyond that, by `FS_EXPLORER_MAX_STEPS` regardless) so a genuinely unanswerable question still terminates. A failed check call falls open (treated as "accept the stop") rather than blocking the run
...
- `FS_EXPLORER_RERANK_ENABLED` (optional, `core-api`, default `true`) — kill switch for `rerank.py`'s cross-encoder reranking step; reranking is also inert (falls back to the heuristic ranker) whenever `OPENROUTER_API_KEY` isn't set, regardless of this flag
- `FS_EXPLORER_RERANK_MODEL` (optional, `core-api`, default `cohere/rerank-4-pro`) — OpenRouter rerank model id (`POST /rerank`); swap to `cohere/rerank-4-fast` for lower latency/cost at some accuracy cost
- `FS_EXPLORER_RERANK_TIMEOUT_SECONDS` (optional, `core-api`, default 8) — fail-open HTTP timeout for the optional rerank optimization; a timeout falls back to retrieval scores and never fails or stops the research workflow
