...
- `OPENROUTER_MAX_OUTPUT_TOKENS` (optional, `core-api`, default `8000`) — `max_tokens` sent with every OpenRouter chat completion request. Reasoning-heavy models (e.g. `z-ai/glm-5.2`) spend completion tokens on hidden chain-of-thought before emitting visible content; without an explicit ceiling they fall back to whatever (often much smaller) default the upstream provider applies and can exhaust it entirely on thinking, returning `finish_reason: "length"` with empty `content` — surfacing as "OpenRouter returned no structured content" even though the request itself succeeded
